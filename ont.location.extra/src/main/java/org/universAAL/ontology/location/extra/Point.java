...
/**
 * The ontological object Point represents a point using GPS coordinates (i.e.
...
public class Point extends ManagedIndividual {
	...
	return new String[] { PROP_ALTITUDE, PROP_LATITUDE, PROP_LONGITUDE };
    }
	...
	if (PROP_ALTITUDE.equals(propURI) || PROP_LATITUDE.equals(propURI)
     ...
     *         altitude(meters)}.If the point altitude property is missing the
	...
	double[] get3DCoordinates = {
		((Double) props.get(PROP_LATITUDE)).doubleValue(),
	...
	double[] get2DCoordinates = {
		((Double) props.get(PROP_LATITUDE)).doubleValue(),
		((Double) props.get(PROP_LONGITUDE)).doubleValue() };
	return get2DCoordinates;
    }

    /**
     * @return the point altitude(meters).If the point altitude property is
     *         missing the method assumes that it is zero.
     */
     ...
     *            (meters)
     */
    public void set3DCoordinates(double latitude, double longitude,
	    double altitude) {
	props.put(PROP_LATITUDE, new Double(latitude));
	props.put(PROP_LONGITUDE, new Double(longitude));
	props.put(PROP_ALTITUDE, new Double(altitude));
	...
	return props.containsKey(PROP_LATITUDE)
		&& props.containsKey(PROP_LONGITUDE);
    }
