 ...
 * it is not possible to create a Place without a place type or with an arbitrary place type.
 * It is possible to create a place instance using the following subclasses:
 * BuildindPlace, RoomPlace, and OtherPlace
 * 
 * 
 * @author smazzei
 *
...
public abstract class Place extends ManagedIndividual {
	...
	static {
		...
		//a place can be connected to another place (i.e. there is a door that connects them)
	 ...
	 * @param uri object URI
	 * @param type PlaceType
	 * @param name The object name property
	 */
	...
	/**
	 * @return true if: <ul> 	<li> HAS_CONNECTION and IS_ADJACENT_TO are symmetric
	 * 							<li> two connected places are also adjacent
	 * 							<li> the Place has an absolute position or it is contained in a place that has an absolute position
	 * 
	 */
	public boolean isWellFormed() {
		//if HAS_CONNECTION is not symmetric or connected places are not adjacent return false
		...
		for (int i=0; i<ConnectedPlacesLenght; i++)
			if ((!ConnectedPlaces[i].isConnectedTo(this))|| (!this.isAdjacentTo(ConnectedPlaces[i])))
				return false;
	...
	/**
	 * 
	 * @return The "is_in_place property as a Place object. This value can be null
	 */
	 ...
	 * @param containingPlace A null value is not admitted as argument
	 */
	...
	/**
	 * Remove is_in_place property. This method also remove the current place from the
	 * "Contains Place" property of argument place.
	 */
	 ...
	 * @param Name A null value or an empty string are not admitted
	 */
	...
	/**
	 * 
	 * @return The PlaceType property. For a well formed place this property is ever present
	 */
	...
	/**
	 * Add a new connection. This is a symmetric property, so the current place is added
	 * in the argument place has_connection property. Two connected places must be adjacent,
	 * so the argument place is also added in IS_ADJACENT_TO property.
	 * If two places are already connected no new connection is created.
	 * @param connectedPlace A null value is not allowed
	 */
	public void addConnectedPlace (Place connectedPlace){
		if (addMultipleValuePlaceProperty(PROP_HAS_CONNECTION, connectedPlace))
		{
			connectedPlace.addMultipleValuePlaceProperty(PROP_HAS_CONNECTION, this);
			this.addAdiacentPlace(connectedPlace);
		}
	}
	
	/**
	 * Remove a connection. This is a symmetric property, so the current place is removed
	 * from the argument place has_connection property
	 * @param noMoreConnectedPlace If the argument is a not connected place or a null object 
	 * no action is performed
	 */
	public void removeConnectedPlace (Place noMoreConnectedPlace)
	{
		if (removeMultipleValuePlaceProperty(PROP_HAS_CONNECTION, noMoreConnectedPlace))
			noMoreConnectedPlace.removeMultipleValuePlaceProperty(PROP_HAS_CONNECTION, this);
	}

	
	/**
	 * Add a new adjacent place. This is a symmetric property, so the current place is added
	 * in the argument place is_adiacent_to property
	 * @param adiacentPlace
	 */
	public void addAdiacentPlace (Place adiacentPlace){
		if (addMultipleValuePlaceProperty(PROP_IS_ADJACENT_TO, adiacentPlace))
			adiacentPlace.addMultipleValuePlaceProperty(PROP_IS_ADJACENT_TO, this);
	}
	
	/**
	 * Remove an adjacent place. This is a symmetric property,
	 * so the current place is removed from the argument place is_adiacent_to property.
	 * Two not adjacent places cannot be connected, so if they are connected the connection
	 * is removed.
	 * @param noMoreAdiacentPlace If the argument is a not adjacent place or a null object 
	 * no action is performed
	 */
	public void removeAdiacentPlace (Place noMoreAdiacentPlace)
	{
		if (removeMultipleValuePlaceProperty(PROP_IS_ADJACENT_TO, noMoreAdiacentPlace))
			noMoreAdiacentPlace.removeMultipleValuePlaceProperty(PROP_IS_ADJACENT_TO, this);
	}
	 ...
	 * @param propURI It can be PROP_HAS_CONNECTION or PROP_IS_ADJACENT_TO
	 * @param place If it is null no property is added.
	 * @return false if no property is added.
	 */
	private boolean addMultipleValuePlaceProperty(String propURI, Place place){
		if (place==null || (propURI!=PROP_HAS_CONNECTION && propURI!=PROP_IS_ADJACENT_TO && propURI!=PROP_CONTAINS_PLACE))
			return false;
		List places;
	 ...
	 * (it depends on the propURI argument) 
	 * @param propURI It can be PROP_HAS_CONNECTION or PROP_IS_ADJACENT_TO
	 * @param place If it is null no property is removed.
	 * @return false if no property is removed.
	 */	private boolean removeMultipleValuePlaceProperty(String propURI, Place noMoreConnectedPlace){
		if (noMoreConnectedPlace==null || 
			(propURI!=PROP_HAS_CONNECTION && propURI!=PROP_IS_ADJACENT_TO && propURI!=PROP_CONTAINS_PLACE)
	...
	/**
	 * 
	 * @param connectedPlace If the argument is a null object the method returns false
	...
	/**
	 * 
	 * @param adjiacentPlace If the argument is a null object the method returns false
	 ...
	 */
	 ...
	 {
		if (removeMultipleValuePlaceProperty(PROP_CONTAINS_PLACE, containedPlace)
			&& containedPlace.isInPlace(this))
	...
	/**
	 * 
	 * @return The place global position (i.e. GPS coordinates). If the place has not
	 * the has_position property or if it is a relative position, the method gets the
	 * containing place position. For a well formed place this method doesn't return
	 * a null object.
	 *  
	 */
	...
	/**
	 * 
	 *@return the room relative position. We can have the following cases: <ol>
	 *	<li>	The HAS_POSITION property is present and it is a relative position.
	 *			In this case the method returns this position.
	 *	<li>	The HAS_POSITION property is present and it is an absolute position.
	 *			In this case the Place position is the same of its referring system origin,
	 *			then the method returns a relative position with X=Y=Z=0
	 *	<li>	The HAS_POSITION property is NOT present and the IS_IN_PLACE property is present.
	 *			In this case the containing place relative position is returned. 
	 *	<li>	The HAS_POSITION property is NOT present and the IS_IN_PLACE property is NOT present.
	 *			In this case the place is not well formed and the method returns null.
	 */
	 public RelativePosition getRelativePosition(){
		...
		if (((Position)props.get(PROP_HAS_POSITION)).getClass().equals(RelativePosition.class))
	 ...
	 * @param pos A null object is not allowed
	 */
	 ...
	 public void setRelativePosition(double x,double y, double z){
		RelativePosition pos = new RelativePosition(new Double(x),new Double(y),new Double(z));
		props.put(PROP_HAS_POSITION, pos);
	}
	 ...
	 */
	 public void setRelativePosition(double x,double y){
		RelativePosition pos = new RelativePosition(new Double(x),new Double(y));
		props.put(PROP_HAS_POSITION, pos);
	}
	 ...
	 * 
	 */
	 public void setAbsolutePosition(double latitude,double longitude, double altitude){
		 AbsolutePosition pos = new AbsolutePosition(latitude,longitude,altitude);
		 props.put(PROP_HAS_POSITION, pos);
	 }
	 
	
	/**
	 * Get the place PhysicalAddress. If the place has no physical address the containing
	 * place physical address is returned (if it is present)
	 ...
	 * @param address A null object is not allowed.
	 */
	...
	public FloorSwitcherPlace[] getFloorSwitchers()
	{
		...
		return (FloorSwitcherPlace[])FloorSwitchers.toArray(new FloorSwitcherPlace[0]);
	}
