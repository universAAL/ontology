...
public class PLocation extends Location {
	...
	static {
		...
		PROP_TARGET_TEMPERATURE = uAAL_VOCABULARY_NAMESPACE + "target_temperature";
	 ...
	 * @param position An AbsolutePosition that represents the user GPS position
	 */
	...
	* Creates a new Location object
	* @param uri The Location instance URI
	* @param place The place where the user is located. A null value or a not well formed
	* place are not allowed.
	*/
	...
	*/
	...
	/**
	 * 
	 * @return The value of "is in place" property. If the property is not present a null
	 * object is returned.
	 */
	 ...
	 * @param place A null value is not admitted.
	 */
	...
	/**
	 * 
	 * @return The value of "has position" property. If the property is not present a null
	 * object is returned.
	 */
	...
	/**
	 * Set the "has position" property using an absolute position (i.e. GPS coordinates)
	 */
	...
	/**
	 * Set the "has position" property using a relative position (it is possible to use
	 * a relative position only when the "is in place" attribute is present)
	 */
	...
	/**
	 * 
	 * @return The value of "is near address" property. If the property is not present a null
	 * object is returned.
	 */
	...
	public float getDistanceTo(Location other) {
		...
		if (props.containsKey(PROP_IS_IN_PLACE) && otherPLoc.props.containsKey(PROP_IS_IN_PLACE))
	...
	/**
	 * 
	 * Get the distance between 2 points represented using longitude/latitude pairs
	 */
	public static double getDistanceFromLatLong(double[] current2DCoordinatesDeg, double[] other2DCoordinatesDeg)
	{
		...
		double [] current2DCoordinatesRad = {Math.PI*current2DCoordinatesDeg[0]/180, Math.PI*current2DCoordinatesDeg[1]/180};
		double [] other2DCoordinatesRad = {Math.PI*other2DCoordinatesDeg[0]/180, Math.PI*other2DCoordinatesDeg[1]/180};
		
		//calculate fi, the absolute value of the two longitudes
		
		double fi= Math.abs(current2DCoordinatesRad[1]-other2DCoordinatesRad[1]);
		
		//calculate p, the distance in radiant (applying an Eulero's theorem)
		
		double p =Math.acos(Math.sin(current2DCoordinatesRad[0])*Math.sin(other2DCoordinatesRad[0])+
					Math.cos(current2DCoordinatesRad[0])*Math.cos(other2DCoordinatesRad[0])*Math.cos(fi));
		
		//calculate the required distance by multiplying the radiant distance with the 
	...
	private double getGPSDistanceTo(PLocation other){
		...
		return getDistanceFromLatLong(current2DCoordinatesDeg,other2DCoordinatesDeg);
	}
	...
	/**
	 * Returns the distance between two locations that have not absolute position
	 */
	private double getMetricDistanceTo(PLocation other){
		...
		RelativePosition currentPlacePosition = currentPlace.getRelativePosition();
		RelativePosition otherPlacePosition = otherPlace.getRelativePosition();
		
		if (currentOrigin == null || otherOrigin == null)
			return NOT_COMPUTABLE_DISTANCE;
		
		//if relative positions uses different systems return the distance between 
		// places absolute positions.
		if (!currentOrigin.equals(otherOrigin))
			return getDistanceFromLatLong(currentOrigin.get2DCoordinates(), otherOrigin.get2DCoordinates());
		
		// get relative positions
		double [] currentRelativeXYZ=getRelativeXYZ();
		double [] otherRelativeXYZ = other.getRelativeXYZ();
		double Z_distance=Math.abs(currentRelativeXYZ[2]-otherRelativeXYZ[2]);
		
		//Z_roomDistance is used to check if the two locations are on the same floor
		double Z_roomDistance=-1;
		if (currentPlacePosition!=null && otherPlacePosition!=null)
			Z_roomDistance=Math.abs(currentPlacePosition.get3DCoordinates()[2]-
		...
		for (int i=0; i<switchers.length; i++)
		{
			...
			{
				currentZMatch= currentZMatch||((currentRelativeXYZ[2]-rz[j].doubleValue())<ROOMS_MEDIUM_HEIGHT
								&& currentRelativeXYZ[2]>=rz[j].doubleValue());
				otherZMatch= otherZMatch||((otherRelativeXYZ[2]-rz[j].doubleValue())<ROOMS_MEDIUM_HEIGHT
				&& otherRelativeXYZ[2]>=rz[j].doubleValue());
			}
			...
			double tempDinst;
			double [] temp2DCoord=switchers[i].getRelativePosition().get2DCoordinates();
	...
	/**
	 * 
	 * @return the current location relative position. If a relative position is present, it is returned.
	 * If "is in place" is present, the place relative coordinates are returned.
	 * <br>The method returns {0,0,0} if no relative position is present.
	 */
	public double[] getRelativeXYZ(){
		RelativePosition pos = null;

		if (!props.containsKey(PROP_HAS_POSITION) && props.containsKey(PROP_IS_IN_PLACE))
	...
	/** 
	* @return the current location absolute position (long/lat). Than, if an absolute position is
	* present, it is returned.
	* If "is in place" is present, the place absolute coordinates are returned.
	 * <br>The method return null if no absolute position is present.
	 */
	...
	/**
	 * Get the distance between a location with absolute position and another one with relative
	 * 
	 */
	private double getMixedDistanceTo(PLocation other){
		...
		if (currentAbsoluteLatLong==null || otherAbsoluteLatLong==null)
			return NOT_COMPUTABLE_DISTANCE;
		return getDistanceFromLatLong(currentAbsoluteLatLong, otherAbsoluteLatLong);		
	}
	...
	/**
	 * A location is well formed when every attribute is well formed and at least one of the
	 * following conditions is respected:<ol>
	 * <li> <i> has position </i> attribute is present and it is an absolute position
	 * <li> <i>is in place </i> attribute is present
	 */
	public boolean isWellFormed() {
		if (!hasAbsolutePosition && !props.containsKey(PROP_IS_IN_PLACE))
			return false;
		if (props.containsKey(PROP_IS_IN_PLACE) && !getIsInPlace().isWellFormed())
			return false;
		if (props.containsKey(PROP_HAS_POSITION)&& !getPosition().isWellFormed())
			return false;
		if (props.containsKey(PROP_IS_NEAR_ADDRESS) && !getIsNearAddress().isWellFormed())
			return false;
		if (props.containsKey(PROP_HAS_TEMPERATURE)&& !getTemperature().isWellFormed())
			return false;
		if (props.containsKey(PROP_TARGET_TEMPERATURE)&& !getTargetTemperature().isWellFormed())
			return false;
	 ...
	 * 			<b>NOT_COMPARABLE_LOCATIONS</b> if two locations are not comparable.
	 */
