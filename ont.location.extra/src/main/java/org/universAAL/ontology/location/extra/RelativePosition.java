...
/**
 * A RelativePosition represents a point position in the current referring system
 * @author smazzei
 *
 */
public class RelativePosition extends ManagedIndividual implements Position{
	...
	*/
	...
	/**
	 * 
	 * @return the double array {X, Y, Z}.If the point Z property is missing the method
	 * assumes that it is zero. For a well formed relative position this method doesn't
	...
	public double [] get3DCoordinates(){
		...
		double [] get3DCoordinates = {((Double)props.get(PROP_X)).doubleValue(), ((Double)props.get(PROP_Y)).doubleValue(), altitude};
		return get3DCoordinates;
	}

	/**
	* 
	* @return the double array {X, Y}. For a well formed relative position this method
	* doesn't return a null object.
	*/
	...
	/**
	 * A RelativePosition is well formed if at least X and Y coordinates are present.
	 */
	public boolean isWellFormed() {
		return props.containsKey(PROP_X)
		&& props.containsKey(PROP_Y);
	}

	/**
	 * @return the relative position accuracy (i.e. X/Y/Z accuracy).
	 * If this property is not present a null value is returned.
	 */
