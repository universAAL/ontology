...
public class ContinuaHealthManagerOntology extends Ontology {
	...
	public void create() {
		...
		oci.addRestriction(MergedRestriction.getAllValuesRestrictionWithCardinality(ContinuaHealthDevice.PROP_HAS_MAC,
		...
		oci.addRestriction(MergedRestriction.getAllValuesRestrictionWithCardinality(
				ContinuaHealthDevice.PROP_HAS_DATA_TYPE, TypeMapper.getDatatypeURI(String.class), 1, 1));
		...
		oci.addSuperClass(ManagedIndividual.MY_URI);
		oci.addObjectProperty(ContinuaHealthManager.PROP_HAS_CONTINUA_DEVICE).setFunctional();
		oci.addRestriction(MergedRestriction.getAllValuesRestriction(ContinuaHealthManager.PROP_HAS_CONTINUA_DEVICE,
				ContinuaHealthDevice.MY_URI));
		oci.addObjectProperty(ContinuaHealthManager.PROP_HAS_NOT_CONTINUA_DEVICE).setFunctional();
		oci.addRestriction(MergedRestriction.getAllValuesRestriction(ContinuaHealthManager.PROP_HAS_NOT_CONTINUA_DEVICE,
				ContinuaHealthDevice.MY_URI));
	}
}