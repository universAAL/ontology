...
public class Questionnaire extends ManagedIndividual {
	...
	public Questionnaire (String name, String description, Question[] questions){
		...
		this.setOrderedQuestions(true); // by default, the questions within a questionnaire are ordered.
	}
	 ...
	 * @param questions, the array of questions that composes the questionnaire
	 * @see Question
	 */
	 ...
	 * @param question, the array of questions that composes the questionnaire
	 * @see Question
	 */
	 ...
	 * @param name, the questionnaire's name ({@link String})
	 */
	...
	/**
	 * The following method sets the name of a questionnaire
	 * @param qDescription, the questionnaire's name ({@link String})
	 */
	...
	/**
	 * The following method checks if the questions within
	 * a questionnaire are ordered.
	...
	/**
	 * The following method sets if the questions within
	 * a questionnaire are ordered or not.
	 ...
	 * @return false if the questionnaire doesn't contain single-choice questions
	...
	/**
	 * The following method returns all the ConditionalChoiceQuestions within
	 * a questionnaire.
	...
	/**
	 * The following method returns all the MultiChoiceQuestions within
	 * a questionnaire. Although a conditional question can be itself also
	 * a multi-choice or single-choice question, we consider them separately.
	...
	/**
	 * The following method returns all the SingleChoiceQuestions within
	 * a questionnaire. Although a conditional question can be itself also
	 * a multi-choice or single-choice question, we consider them separately.
	...
	/**
	 * The following method returns all the QuestionWithMedia within
	 * a questionnaire. Although a question with media can be itself also
	 * a multi-choice or single-choice question, we consider them separately.
	...
	/**
	 * The following method gets the number of type of
	 * question, within the questionnaire.
	 * @param uri, that indicates whether the question is 
	 * 		MultiChoice, SingleChoice, Conditional or WithImage
	...
	public Question writeASingleChoiceQuestion(String qWording, String answerType){
		SingleChoiceQuestion scq = new SingleChoiceQuestion(qWording, answerType);
		addQuestion(scq);
		return scq;
	}

	public Question writeASingleChoiceQuestion(String qWording, String answerType,ChoiceLabel[] choices){
		SingleChoiceQuestion scq = new SingleChoiceQuestion(qWording,answerType,choices);
		addQuestion(scq);
		return scq;
	}

	public Question writeASingleChoiceQuestion(String qWording, Object correctAnswer, String answerType, ChoiceLabel[] choices){
		SingleChoiceQuestion scq = new SingleChoiceQuestion(qWording, correctAnswer, answerType, choices);
		addQuestion(scq);
		return scq;
	}
	...
	public Question writeAMultiChoiceQuestion(String qWording, String answerType,ChoiceLabel[] choices){
		MultiChoiceQuestion mcq = new MultiChoiceQuestion(qWording, answerType, choices);
		addQuestion(mcq);
		return mcq;
	}

	public Question writeAMultiChoiceQuestion(String qWording, Object[] correctAnswer, String[] answerType, ChoiceLabel[] choices){
		MultiChoiceQuestion mcq = new MultiChoiceQuestion(qWording, correctAnswer, answerType, choices);
		addQuestion(mcq);
		return mcq;
	}
	...
	public Question writeAConditionalQuestion(String qWording, Object triggerAnswer, Question dependingQuestion, String answerType){
		ConditionalQuestion cq = new ConditionalQuestion(qWording, triggerAnswer, dependingQuestion, answerType);
		addQuestion(cq);
		return cq;
	}
	...
	public Question writeAConditionalQuestion(String qWording, Object triggerAnswer, Question dependingQuestion, String answerType, ChoiceLabel[] choices ){
		ConditionalQuestion cq = new ConditionalQuestion(qWording, triggerAnswer, dependingQuestion, answerType, choices );
		addQuestion(cq);
		return cq;
	}
	...
	public Question writeAConditionalQuestion(String qWording, Object triggerAnswer, Question dependingQuestion, String answerType, ChoiceLabel[] choices, Object correctAnswer){
		ConditionalQuestion cq = new ConditionalQuestion(qWording, triggerAnswer, dependingQuestion, answerType, choices, correctAnswer);
		addQuestion(cq);
		return cq;
	}
	...
	public Question writeQuestionWithMedia(MediaObject image, String questionWording, String answerType){
		QuestionWithMedia qwm = new QuestionWithMedia(image, questionWording, answerType); 
		addQuestion(qwm);
		return qwm;
	}

	public Question writeQuestionWithMedia(String qWording, String answerType,ChoiceLabel[] choices, MediaObject image){
		QuestionWithMedia qwm = new QuestionWithMedia(qWording, answerType,choices, image); 
		addQuestion(qwm);
		return qwm;
	}
	...
	public Question writeQuestionWithMedia(String qWording, Object correctAnswer, String answerType, ChoiceLabel[] choices, MediaObject image){
		QuestionWithMedia qwm = new QuestionWithMedia(qWording, correctAnswer, answerType, choices, image); 
		addQuestion(qwm);
		return qwm;
	}
	...
	public String questionnaireToString(){
		...
		for(int i=0;i<questions.length;i++){
			writtenQuestionnaire = writtenQuestionnaire + "Question " + index + ": " + questions[i].getQuestionWording() + "\n";
			...
			if(questions[i] instanceof ConditionalQuestion){
				...
				if(ta.length==1){
					writtenQuestionnaire = writtenQuestionnaire + "Answer this question ONLY if you have answered " +  "\"" +
					cq.getDependsOn().getLabel(ta[0]) + "\"" +  " to question " + "\"" + cq.getDependsOn().getQuestionWording()+ 
					"\"" + "\n";
				}
					...
					for(int k=0;k<ta.length;k++){
						writtenQuestionnaire = writtenQuestionnaire  + "\"" + cq.getDependsOn().getLabel(ta[k])+ "\"" ;
						if(!(k==(ta.length-1))){
			...
			for (int j=0;j<nChoices;j++){
				...
				writtenQuestionnaire = writtenQuestionnaire + choices[j].getLabel() + "\n"; 
			}
