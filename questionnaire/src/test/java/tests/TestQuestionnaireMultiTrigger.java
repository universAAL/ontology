...
/**
 * The following test class will check that
 * the conditional questions associated to a multi-
 * trigger answer work properly.
 * @author mdelafuente
 *
 */

public class TestQuestionnaireMultiTrigger extends SuperTestQuestionnaire{
	...
	public void setUp(){
		...
		q1 = new SingleChoiceQuestion("Is tobacco bad for health?", Boolean.TRUE, TypeMapper.getDatatypeURI(Boolean.class), choices2);
		q2 = new MultiChoiceQuestion("When do you have to take your pills?",correctAnswers,correctAnswersURI,choices1);
		
		q3 = new ConditionalQuestion("Are you sure?", Boolean.FALSE, q1,TypeMapper.getDatatypeURI(Boolean.class), choices2);
		q4 = new ConditionalQuestion("Do you smoke?", Boolean.TRUE, q1, TypeMapper.getDatatypeURI(Boolean.class), choices2);
		
		q5 = new ConditionalQuestion("Do you agree with this schedule?", correctAnswers, q2, TypeMapper.getDatatypeURI(Boolean.class), choices2);
	...
	public void testMultiTriggerCQAnswersMessy(){
		AnsweredQuestionnaire aq = new AnsweredQuestionnaire(questionnaire, peter);
		aq.answeredToQuestion(q1, Boolean.TRUE);
		Object[] mcqAnswers = {PartsOfDay.IN_THE_EVENING, PartsOfDay.IN_THE_MORNING};
		aq.answeredToQuestion(q2, mcqAnswers);
		aq.answeredToQuestion(q4,Boolean.TRUE);
		aq.answeredToQuestion(q5,Boolean.TRUE);
	 ...
	 * correction, even if the answers given in the multi-choice question
	 * are not in the same order as expected. 
	 * In this case, half of the questions with correct answer
	 * has been answered properly, and the other half, wrongly.
	 */
	@Test
	public void testMultiTriggerCQAnswersIncorrect(){
		AnsweredQuestionnaire aq = new AnsweredQuestionnaire(questionnaire, peter);
		aq.answeredToQuestion(q1, Boolean.TRUE);
		Object[] mcqAnswers = {PartsOfDay.IN_THE_AFTERNOON, PartsOfDay.IN_THE_MORNING};
		aq.answeredToQuestion(q2, mcqAnswers);
		aq.answeredToQuestion(q4,Boolean.TRUE);
		aq.answeredToQuestion(q5,Boolean.TRUE);
