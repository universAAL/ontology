...
public class DroolsReasoningOntology extends Ontology {
	...
	oci.addRestriction(MergedRestriction.getAllValuesRestrictionWithCardinality(ConsequenceProperty.PROP_HAS_KEY,
		TypeMapper.getDatatypeURI(String.class), 1, 1));
	oci.addDatatypeProperty(ConsequenceProperty.PROP_HAS_VALUE).setFunctional();
	oci.addRestriction(MergedRestriction.getAllValuesRestrictionWithCardinality(ConsequenceProperty.PROP_HAS_VALUE,
		TypeMapper.getDatatypeURI(String.class), 1, 1));
	// Consequence
	...
	oci.addRestriction(MergedRestriction.getAllValuesRestrictionWithCardinality(Consequence.PROP_HAS_IDENTIFIER,
		TypeMapper.getDatatypeURI(Long.class), 1, 1));
	oci.addObjectProperty(Consequence.PROP_HAS_PROPERTIES);
	oci.addRestriction(
		MergedRestriction.getAllValuesRestriction(Consequence.PROP_HAS_PROPERTIES, ConsequenceProperty.MY_URI));
	// FactProperty
	...
	oci.addRestriction(MergedRestriction.getAllValuesRestrictionWithCardinality(FactProperty.PROP_HAS_KEY,
		TypeMapper.getDatatypeURI(String.class), 1, 1));
	oci.addDatatypeProperty(FactProperty.PROP_HAS_VALUE).setFunctional();
	oci.addRestriction(MergedRestriction.getAllValuesRestrictionWithCardinality(FactProperty.PROP_HAS_VALUE,
		TypeMapper.getDatatypeURI(String.class), 1, 1));
	// Fact
	oci = createNewOntClassInfo(Fact.MY_URI, factory, 1);
	oci.setResourceComment(
		"A element which models some characteristics of the reality in a service or application.");
	...
	oci.setResourceComment(
		"A rule written in drools format for expressing a group of conditions which combined with a set of facts and input events results as a consequence firing.");
	oci.setResourceLabel("Rule");
	oci.addSuperClass(ManagedIndividual.MY_URI);
	oci.addDatatypeProperty(Rule.PROP_HAS_IDENTIFIER).setFunctional();
	oci.addRestriction(MergedRestriction.getAllValuesRestrictionWithCardinality(Rule.PROP_HAS_IDENTIFIER,
		TypeMapper.getDatatypeURI(Long.class), 1, 1));
	oci.addDatatypeProperty(Rule.PROP_HAS_BODY).setFunctional();
	oci.addRestriction(
		MergedRestriction.getAllValuesRestriction(Rule.PROP_HAS_BODY, TypeMapper.getDatatypeURI(String.class)));
	// DroolsReasoningService
	oci = createNewOntClassInfo(DroolsReasoning.MY_URI, factory, 5);
	oci.setResourceComment(
		"The main service to request reasoning throught the Drools Business Rules Process Management.");
	...
	oci.addRestriction(MergedRestriction.getAllValuesRestriction(DroolsReasoning.PROP_PRODUCES_CONSEQUENCES,
		Consequence.MY_URI));
    }
