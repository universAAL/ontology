...
public class LineSegment extends Line {
	...
	public float getDistanceTo(Point point) {
		...
		CoordinateSystem common = CoordinateSystem.findCommonParentSystem(point.getCoordinateSystem(), this.getLocalCoordinateSystem());
		...
		JgclPointOnCurve3D[] proj = (new JgclLine3D(new JgclCartesianPoint3D(a),new JgclCartesianPoint3D(b)).projectFrom(c));
		float dist = Float.MAX_VALUE;
		for(int i=0;i<proj.length;i++) {
			double curdist;
			if((curdist = proj[i].distance(c)) < dist) dist = (float)curdist;
		}
		return dist;
	}
	/**
	...
	public float getDistanceTo(Shape shape) {
		...
		if(shape instanceof Box) {
			...
			Point end = getEnd();
			Point dir = new Point(end.getX()-start.getX(),end.getY()-start.getY(),end.getZ()-start.getZ(),start.getCoordinateSystem());
	...
	private float distanceRay(Point lineStart,Point lineDir,Box box,float minDir, float maxDir) {
		
		// at first, the common parent coordinate system has to be found
		CoordinateSystem common = CoordinateSystem.findCommonParentSystem(lineStart.getCoordinateSystem(), box.getLocalCoordinateSystem());
		
		// then the direction of the line segment and its start point have to be transformed to the common coordinate system
		lineDir = lineDir.getInHigherCoordinateSystem(common);
		lineStart = lineStart.getInHigherCoordinateSystem(common);
		
		// a new point with the inverted components of the direction point will be created
		Point invDir = new Point((float)(1f / lineDir.getX()),(float)( 1f / lineDir.getY()),(float)(1f / lineDir.getZ()),new CoordinateSystem());
		...
		// creates arrays with the minimum and maximum values for the x, y and z components of the box shape
		double[] min = {center.getX()-box.getWidth()/2f, center.getY()-box.getDepth()/2f, center.getZ()-box.getHeight()/2f};
		double[] max = {center.getX()+box.getWidth()/2f, center.getY()+box.getDepth()/2f, center.getZ()+box.getHeight()/2f};
		...
		// 
		double pt[] = {tmin*lineDir.getX(),tmin*lineDir.getY(),tmin*lineDir.getZ()};
