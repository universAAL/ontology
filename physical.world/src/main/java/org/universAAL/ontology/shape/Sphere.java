...
public class Sphere extends Shape3D {
	...
	/**
	 * create a new Sphere
	 * @param uri the universal identifier
	...
	public float getDistanceTo(Shape shape) {
		...
		if(shape instanceof Sphere) {
			CoordinateSystem commonsys = CoordinateSystem.findCommonParentSystem(getLocalCoordinateSystem(), shape.getLocalCoordinateSystem());
			if(commonsys == null) throw new IllegalArgumentException("Not part of the same coordinate system chain");
			dist = Math.abs(((Sphere)shape).getCenter().getInHigherCoordinateSystem(commonsys).getDistanceTo(getCenter().getInHigherCoordinateSystem(commonsys)));
			dist -=  ((Sphere)shape).getRadius();
			dist -= getRadius();
			if(dist < 0) dist = Point.INTERSECTING;
			return dist;
		}
	...
	public float getDistanceTo(Point point) {
		CoordinateSystem commonsys = CoordinateSystem.findCommonParentSystem(getLocalCoordinateSystem(), point.getCoordinateSystem());
		if(commonsys == null) throw new IllegalArgumentException("Not part of the same coordinate system chain");
		float dist = Math.abs(point.getInHigherCoordinateSystem(commonsys).getDistanceTo(getCenter().getInHigherCoordinateSystem(commonsys)));
		dist -= getRadius();
	...
	public boolean contains(Shape3D shape) {
		if(shape instanceof Sphere) {
			if(getRadius() < ((Sphere)shape).getRadius()) return false;
			float dist = getDistanceTo(shape.getCenter());
			if(dist > 0) return false;
			if(Math.abs(dist)+((Sphere)shape).getRadius() < getRadius()) return true;
		}
		if(shape instanceof Box) {
			...
			double halfs[] = {((Box)shape).getWidth()/2d,((Box)shape).getDepth()/2d,((Box)shape).getHeight()/2d};
			for(int i=0;i<3;i++) {
				if(halfs[i] > radius) return false;
				if(dist[i] > (radius-halfs[i])) return false;
			}
			return true;
		}
		return false;
