...
public class Box extends Shape3D {
	...
	/** assumes the given coordinate system to be centered at the lower left corner of the object,
	 * not the center. Note: the coordinate system will be modified !
	 * @param uri
	 * @param width
	 * @param height
	 * @param depth
	 * @param system the coordinate system centered at the lower left corner
	 * @return a Box with the given values.
	 */
	public static Box createByCorner(String uri, double width,double depth,double height,OriginedMetric system) {
		Point p = system.getOrigin();
		p.set3DCoordinates(p.getX()+width/2d, p.getY()+depth/2d, p.getZ()+height/2d);
		system.setOrigin(p);
	 ...
	 * @param height
	 * @param depth
	 * @param parent the containing place
	 * @return a Box with the given values.
	 */
	public static Box createByCorner(String uri, double width,double height,double depth,Place parent) {
		OriginedMetric om = new OriginedMetric(width/2f, depth/2f, height/2f,parent);
	...
	public Box(String uri, double width,double height,double depth,CoordinateSystem system) {
		super(uri,system);
	...
	public Box(double width,double height,double depth,CoordinateSystem system) {
		super(system);
		props.put(PROP_WIDTH, new Double(width));
		props.put(PROP_HEIGHT, new Double(depth));
		props.put(PROP_DEPTH, new Double(height));
	...
	public float getDistanceTo(Shape shape) {
		...
		if(shape instanceof Sphere) {
			...
			if(dist <=0 ) return Point.INTERSECTING;
			return dist;
		}
		if(shape instanceof Box) {
			CoordinateSystem cs = CoordinateSystem.findCommonParentSystem(getCenter().getCoordinateSystem(), shape.getCenter().getCoordinateSystem());
			double[] center = getCenter().getInHigherCoordinateSystem(cs).get3DCoordinates();
			double[] p = shape.getCenter().getInHigherCoordinateSystem(cs).get3DCoordinates();
			double halfs[] = {getWidth()/2d+((Box)shape).getWidth()/2d,getDepth()/2d+((Box)shape).getDepth()/2d,getHeight()/2d+((Box)shape).getHeight()/2d};
			double dist[] = new double[3];
			for(int i=0;i<3;i++) {
				if(center[i]+halfs[i]>p[i]) dist[i] = Math.max(0, center[i]-halfs[i]-p[i]);
				else dist[i] = p[i]-center[i]-halfs[i];
			}
			if(dist[0] <= 0 && dist[1] <= 0 && dist[2] <= 0) return Point.INTERSECTING;
	...
	public float getDistanceTo(Point point) {
		...
		float realDist = (float) Math.sqrt(Math.pow(dist[0], 2)+Math.pow(dist[1], 2)+Math.pow(dist[2], 2));
		if(dist[0] <= 0 && dist[1] <= 0 && dist[2] <= 0) return Point.INTERSECTING;
		else return realDist;
	}
	...
	public String toString() {
		return "[Center:"+getCenter().getX()+","+getCenter().getY()+","+getCenter().getZ()+"|Size:"+getWidth()+","+getDepth()+","+getHeight()+"]";
	}
	...
	public boolean contains(Shape3D shape) {
		if(shape instanceof Sphere) {
			...
			if(-dist > ((Sphere)shape).getRadius()) return true;		
		}
		if(shape instanceof Box) {
			CoordinateSystem common = CoordinateSystem.findCommonParentSystem(getLocalCoordinateSystem(),shape.getLocalCoordinateSystem());
			double[] p1 = getCenter().getInHigherCoordinateSystem(common).get3DCoordinates();
			double[] p2 = shape.getCenter().getInHigherCoordinateSystem(common).get3DCoordinates();
			double dist[] = {Math.abs(p1[0]-p2[0]),Math.abs(p1[1]-p2[1]),Math.abs(p1[2]-p2[2])}; 
			double halfs1[] = {getWidth()/2d,getDepth()/2d,getHeight()/2d};
			double halfs2[] = {((Box)shape).getWidth()/2d,((Box)shape).getDepth()/2d,((Box)shape).getHeight()/2d};
			for(int i=0;i<3;i++) {
				if(halfs1[i] < halfs2[i]) return false;
				if(dist[i] > (halfs1[i]-halfs2[i])) return false;
			}
			return true;
		}
		return false;
