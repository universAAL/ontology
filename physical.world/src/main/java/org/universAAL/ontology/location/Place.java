...
public class Place extends Location {
	...
	static {
		...
		PROP_LOCATION_OF_PHYSICAL_THING = uAAL_LOCATION_NAMESPACE + "locationOf";
	 ...
	 * @param uri this value can also be a null object
	 * @param name The place name. A null object is not allowed
	 * @param shape the shape of the place. IS not allowed to have a OriginedMetric as local Coordinate System
	 */
	public Place(String uri, String name,Shape shape) {
		super(uri,name);
//		if(shape.getLocalCoordinateSystem() instanceof OriginedMetric) throw new IllegalArgumentException("Shape is not allowed to have a OriginedMetric, must be unanchored");
...
//		if(shape.getLocalCoordinateSystem() instanceof OriginedMetric) throw new IllegalArgumentException("Shape is not allowed to have a OriginedMetric, must be unanchored");
	 ...
	 * 
	 * 
	 * @param other the Location object
	 * @return a float value which provides information about the distance between the Place object and the Location Object
	 * 
	 */
	public float getDistanceTo(Location other) {
		...
		// if other is an instance of Location, it only can be of the type Place of Point
		if(other instanceof Place)
			// other is an instance of shape,
			// the distance will be calculated as the distance by the bounding volumes
			// between this' shape and other's shape.
			return this.getShape().getDistanceByBoundingVolume(((Place)other).getShape());
		else
			// other is an instance of Point
			// the distance will be calculated between this' shape and the Point "other"
			return this.getShape().getDistanceTo((Point)other);
	}

	/** returns the smallest, contained place that contains the given point
	 * does not consider overlapping Places
	 * @param p
	 * @return the smallest containing place or null if the point is not contained
	 */
	public Place getSmallestContaining(Point p) {
		if(p == null) throw new IllegalArgumentException();
		if(!getShape().contains(p)) return null;
		Location[] locs = getContainedLocations();
		if(locs != null) {
			for(int i=0;i<locs.length;i++) {
				Place place;
				if(locs[i].getClass() != Point.class && (place = ((Place)locs[i]).getSmallestContaining(p)) != null) return place;
			}
		}
		return this;
	}
	 ...
	 * @param place must have a shape
	 * @param x the x origin of this place
	 * @param y the y origin of this place
	 * @param z the z origin of this place
	 */
	public void setContainingLocation(Place place,float x,float y,float z) {
		if (place==null || place.getShape() == null) throw new IllegalArgumentException();
		this.getShape().setLocalCoordinateSystem(new OriginedMetric(x,y,z,place));
	...
	/**
	 * Adds a place this place contains, originated at the given point. The origin is not the center
	 * of the object, but the lower, left corner 
	 * @param place must have a box shape
	 * @param x the x origin of this place
	 * @param y the y origin of this place
	 * @param z the z origin of this place
	 */
	public void addContainedLocationRelativeToCorner(Place location,float x,float y,float z) {
		if (location==null || location.getShape() == null || !(location.getShape() instanceof Box)) throw new IllegalArgumentException();
		super.addContainedLocation(location);
		
		// for adding the lower location in the middle of the higher location, relative to the corner
		// we need the half shape measures of the lower location additional to the coordinates of the corner in the coordinate system of the higher location 
		Box box = (Box)(location.getShape());
		// the association between x,y,z and width, height and depth is right in this line
		location.setContainingLocation(this,(float)(x+box.getWidth()/2f),(float)(y+box.getDepth()/2f),(float)(z+box.getHeight()/2f));
