...
public final class LanguageOntology extends Ontology {
	...
	public void create() {
		...
		OntClassInfoSetup oci = createNewOntClassInfo(GoogleTTSVoiceVariant.MY_URI, factory, 0);
		oci.setResourceComment("Representation of a voice variant as used by Google Text-To-Speech Cloud Service.");
		...
		oci.addRestriction(MergedRestriction.getAllValuesRestrictionWithCardinality(
				GoogleTTSVoiceVariant.PROP_IS_MALE_VOICE, TypeMapper.getDatatypeURI(Boolean.class), 0, 1));

		// adding class for Google TTS language
		oci = createNewOntClassInfo(GoogleTTSLang.MY_URI, factory, 1);
		oci.setResourceComment("Representation of a language as used by Google Text-To-Speech Cloud Service.");
		oci.setResourceLabel("Google TTS Language");
		oci.addSuperClass(ManagedIndividual.MY_URI);
		oci.addDatatypeProperty(GoogleTTSLang.PROP_NAME).setFunctional();
		oci.addRestriction(MergedRestriction.getAllValuesRestrictionWithCardinality(GoogleTTSLang.PROP_NAME,
				TypeMapper.getDatatypeURI(String.class), 1, 1));
		oci.addObjectProperty(GoogleTTSLang.PROP_TTS_SUPPORT_MALE);
		oci.addRestriction(MergedRestriction.getAllValuesRestriction(GoogleTTSLang.PROP_TTS_SUPPORT_MALE,
				GoogleTTSVoiceVariant.MY_URI));
		oci.addObjectProperty(GoogleTTSLang.PROP_TTS_SUPPORT_FEMALE);
		oci.addRestriction(MergedRestriction.getAllValuesRestriction(GoogleTTSLang.PROP_TTS_SUPPORT_FEMALE,
				GoogleTTSVoiceVariant.MY_URI));
