 ...
 * related to answer issues work properly. 
 * @author mdelafuente
 *
 */
public class TestQuestionnaireAnswers extends IntegrationTest{
	...
	public void onSetUp(){
		...
		q1 = new SingleChoiceQuestion("Are vegetables good for improving your health condition?",Boolean.TRUE, TypeMapper.getDatatypeURI(Boolean.class), choices2);
		q2 = new SingleChoiceQuestion("Would you answer one more question?", TypeMapper.getDatatypeURI(Boolean.class), choices2);
		q3 = new ConditionalQuestion("When did your caregiver recommended you to practice your relaxation exercices?", Boolean.TRUE, q2,correctAnswersURI, choices1, correctAnswers);
		q4 = new MultiChoiceQuestion("When do you have to take your pills?",correctAnswers,correctAnswersURI,choices1);
		q5 = new SingleChoiceQuestion("When do you prefer to exercise?", TypeMapper.getDatatypeURI(PartsOfDay.class), choices1);
		q6 = new ConditionalQuestion("Do you usually practice sport at this time?", PartsOfDay.IN_THE_AFTERNOON, q5, TypeMapper.getDatatypeURI(Boolean.class), choices2);
		q7 = new SingleChoiceQuestion("When are you exercising tomorrow?", PartsOfDay.IN_THE_MORNING, TypeMapper.getDatatypeURI(PartsOfDay.class), choices1);
	...
	/**
	 * In the following test all the questions have been answered and all are correct
	 * (the ones that have a correct answer, that is: q1,q3,q4 and q7).
	 */
	

	@Test
	public void testAllCorrectAnswers(){
		AnsweredQuestionnaire aq = new AnsweredQuestionnaire(questionnaire, peter);
		
		aq.answeredToQuestion(q1,Boolean.TRUE);
		aq.answeredToQuestion(q2, Boolean.TRUE);
		Object[] mcqAnswers = {PartsOfDay.IN_THE_MORNING, PartsOfDay.IN_THE_EVENING};
	...
	/**
	 * In the following test all the questions have been answered and all are incorrect
	 * (the ones that have a correct answer, that is: q1,q3,q4 and q7).
	 */
	...
	public void testNoCorrectAnswers(){
		...
		aq.answeredToQuestion(q2, Boolean.TRUE);
		Object[] mcqAnswers = {PartsOfDay.IN_THE_MORNING, PartsOfDay.IN_THE_AFTERNOON};
		aq.answeredToQuestion(q3, mcqAnswers);
		aq.answeredToQuestion(q4, mcqAnswers);
		aq.answeredToQuestion(q5,PartsOfDay.IN_THE_AFTERNOON);
		aq.answeredToQuestion(q6,Boolean.TRUE);
	...
	/**
	 * In the following test all the questions have been answered and some are incorrect
	 * (q3 and q4) and some are correct (q1 and q7).
	 */
	@Test
	...
	/**
	 * The following test, tests the percentages methods.
	 * For this test, we have answerer right all the questions.
	 */
	@Test
	...
	public void testXQuestionGetter(){
		...
		ArrayList<Question> incorrectQuestions = new ArrayList<Question>(Arrays.asList(aq.getAssociatedQuestions(aq.getIncorrectAnswers())));
		ArrayList<Question> correctQuestions = new ArrayList<Question>(Arrays.asList(aq.getAssociatedQuestions(aq.getCorrectAnswers())));
		ArrayList<Question> noCorrectQuestions = new ArrayList<Question>(Arrays.asList(aq.getAssociatedQuestions(aq.getNoCorrectAnswers())));
		
		assertTrue(incorrectQuestions.contains(q3)&& incorrectQuestions.contains(q4)&& incorrectQuestions.contains(q7) && incorrectQuestions.size()==3);
		assertTrue(correctQuestions.contains(q1) && correctQuestions.size()==1);
		assertTrue(noCorrectQuestions.contains(q2)&& noCorrectQuestions.contains(q5)&& noCorrectQuestions.contains(q6)&& noCorrectQuestions.size()==3);
	}
	...
	public void testGetAnswers(){
		...
		for (int i=0;i<questions.length;i++){
			Object[] prueba = questions[i].getCorrectAnswers();
			for (int j=0;j<prueba.length;j++){
				System.out.println("Esto es lo que imprime");
	...
	public void testAnswers(){
		...
		Answer[] answers = aq.getAnswers();
		for (int i=0;i<answers.length;i++){
			System.out.println("Contenidos de las respuestas: ");
			Object[] answersContents = answers[i].getAnswerContent();
			
			for (int j=0;j<answersContents.length;j++){
				System.out.println("Esto es lo que imprime en el método polémico: ");
				System.out.println(answersContents[j]);
