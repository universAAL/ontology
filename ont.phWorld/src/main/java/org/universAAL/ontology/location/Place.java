...
public class Place extends Location {
	...
	public static final String PROP_LOCATION_OF_PHYSICAL_THING = LocationOntology.NAMESPACE + "locationOf";
	...
	public Place(String uri, String name, Shape shape) {
		...
		// new
		// IllegalArgumentException("Shape is not allowed to have a
		// OriginedMetric, must be unanchored");
	...
	public int getPropSerializationType(String propURI) {
		...
		if (PROP_HAS_SHAPE.equals(propURI) || PROP_LOCATION_OF_PHYSICAL_THING.equals(propURI)
	...
	public Place getSmallestContaining(Point p) {
		...
		if (locs != null) {
			for (int i = 0; i < locs.length; i++) {
				Place place;
				if (locs[i].getClass() != Point.class && (place = ((Place) locs[i]).getSmallestContaining(p)) != null)
					return place;
			}
		}
		return this;
	...
	public void addContainedLocationRelativeToCorner(Place location, float x, float y, float z) {
		if (location == null || location.getShape() == null || !(location.getShape() instanceof Box))
			throw new IllegalArgumentException();
		super.addContainedLocation(location);
		...
		// this line
		location.setContainingLocation(this, (float) (x + box.getWidth() / 2f), (float) (y + box.getDepth() / 2f),
