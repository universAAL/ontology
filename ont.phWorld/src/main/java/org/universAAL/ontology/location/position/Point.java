 ...
 * representing an ontological concept in Java classes for universAAL. Usually it
...
public class Point extends Location {
	...
	public int getPropSerializationType(String propURI) {
		...
		if (PROP_X.equals(propURI) || PROP_Y.equals(propURI) || PROP_Z.equals(propURI)
	...
	public double[] get2DCoordinates() {
		double[] get2DCoordinates = { ((Double) props.get(PROP_X)).doubleValue(),
	...
	public float getDistanceTo(Location other) {
		if (other == null || other.getClass() != Point.class)
			throw new IllegalArgumentException();
		CoordinateSystem commonsys = CoordinateSystem.findCommonParentSystem(getCoordinateSystem(),
				((Point) other).getCoordinateSystem());
		...
		if (p1.getCoordinateSystem() == p2.getCoordinateSystem()) {
			if (this.getCoordinateSystem().getURI().indexOf("WGS84") > -1) {
				...
				double[] SolVec = { otherCoords[0] - thisCoords[0], otherCoords[1] - thisCoords[1],
	...
	private double getDistanceFromLatLong(double[] current2DCoordinatesDeg, double[] other2DCoordinatesDeg) {
		...
		double[] current2DCoordinatesRad = { Math.PI * current2DCoordinatesDeg[0] / 180,
				Math.PI * current2DCoordinatesDeg[1] / 180 };
		double[] other2DCoordinatesRad = { Math.PI * other2DCoordinatesDeg[0] / 180,
	...
	public Point getInHigherCoordinateSystem() {
		...
		// .getCoordinateSystem());
		try {
			throw new Exception("The method 'getInHigherCoordinateSystem' is disabled in this version of to "
			...
			System.out.println("The method 'getInHigherCoordinateSystem' is disabled in this version of to "
					+ "avoid GPL libraries. If you really need this method, use the restricted GPL version, or"
					+ "modify source code of this version to include the dependencies you need:"
					+ "j3d-core and vecmath, versions at least 1.3.1.");
			e.printStackTrace();
