...
public class Location extends AbsLocation {
	...
	private boolean addMultipleValueLocationProperty(String propURI, Location location) {
		if (location == null
				|| (propURI != PROP_IS_ADJACENT_TO && propURI != PROP_IS_CONNECTED_TO && propURI != PROP_CONTAINS))
			return false;
		List locations;
	...
	private boolean removeMulitpleValueLocationProperty(String propURI, Location location) {
		if (!props.containsKey(propURI) || location == null || (propURI != PROP_IS_ADJACENT_TO
				&& propURI != PROP_IS_CONNECTED_TO && propURI != PROP_IS_CONTAINED_IN && propURI != PROP_CONTAINS))
			return false;
		List locations;
		if (props.containsKey(propURI)) {
			locations = (List) props.get(propURI);
	...
	public int getPropSerializationType(String propURI) {
		if (PROP_IS_CONNECTED_TO.equals(propURI) || PROP_HAS_NAME.equals(propURI) || PROP_IS_ADJACENT_TO.equals(propURI)
				|| PROP_IS_CONTAINED_IN.equals(propURI) || PROP_CONTAINS.equals(propURI))
		...
		if (current != null) {
			for (int i = 0; i < current.length; i++) {
				if (current[i].equals(compare))
					return true;
				if (recursiveCompare(current[i].getContainedLocations(), compare))
					return true;
			}
		}
		return false;
