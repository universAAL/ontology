...
public class PhysicalThing extends ManagedIndividual {
	...
	public int getPropSerializationType(String propURI) {
		if (PROP_PHYSICAL_LOCATION.equals(propURI) || PROP_HAS_SHAPE.equals(propURI))
	...
	public void setLocationRelativeToCorner(Location o, float x, float y, float z) {
		if (o != null) {
			...
			if (o instanceof Place) {
				...
				((Place) o).addContainedLocation(place, (float) (x - box.getWidth() / 2f),
						(float) (y - box.getDepth() / 2f), (float) (z - box.getHeight() / 2f));
			} else
				place.setContainingLocation(place, x, y, z);
	...
	public void setLocationCornerRelativeToCorner(Location o, float x, float y, float z) {
		if (o != null) {
			...
			if (o instanceof Place) {
				...
				((Place) o).addContainedLocation(place, (float) (x - box.getWidth() / 2f + box2.getWidth() / 2f),
						(float) (y - box.getDepth() / 2f + box2.getDepth() / 2f),
						(float) (z - box.getHeight() / 2f + box2.getHeight() / 2f));
			} else
				place.setContainingLocation(place, x, y, z);
			place.setPhysicalThingofLocation(this);
			props.put(PROP_PHYSICAL_LOCATION, place);
