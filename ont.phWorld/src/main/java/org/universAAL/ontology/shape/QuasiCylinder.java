...
public class QuasiCylinder extends Shape3D {
	...
	private void checkValid() {
		...
		if (head.getLocalCoordinateSystem().getClass() != OriginedMetric.class
				...
				|| ((OriginedMetric) head.getLocalCoordinateSystem()).getOrigin()
						.getCoordinateSystem() != ((OriginedMetric) foot.getLocalCoordinateSystem()).getOrigin()
								.getCoordinateSystem())
	...
	public Point getMidpoint() {
		...
		Point center = new Point((headcenter[0] - footcenter[0]) / 2d + footcenter[0],
				(headcenter[1] - footcenter[1]) / 2d + footcenter[1],
				(headcenter[2] - footcenter[2]) / 2d + footcenter[2], getHead().getCenter().getCoordinateSystem());
		return center;
	}
	...
	protected Shape computeBoundingVolume() {
		...
		Point[] pts = {
				...
				new Point(-axis3 / 2d, -axis4 / 2d, 0, foot.getLocalCoordinateSystem()).getInHigherCoordinateSystem() };
		Point[] bnds = {
				new Point(Math.max(Math.max(Math.max(pts[0].getX(), pts[1].getX()), pts[2].getX()), pts[3].getX()),
						Math.max(Math.max(Math.max(pts[0].getY(), pts[1].getY()), pts[2].getY()), pts[3].getY()),
						Math.max(Math.max(Math.max(pts[0].getZ(), pts[1].getZ()), pts[2].getZ()), pts[3].getZ()),
				...
				new Point(Math.min(Math.min(Math.min(pts[0].getX(), pts[1].getX()), pts[2].getX()), pts[3].getX()),
						...
						Math.min(Math.min(Math.min(pts[0].getZ(), pts[1].getZ()), pts[2].getZ()), pts[3].getZ()),
						getLocalCoordinateSystem()) };
		return new Box(bnds[0].getX() - bnds[1].getX(), bnds[0].getY() - bnds[1].getY(),
				bnds[0].getZ() - bnds[1].getZ(),
				new OriginedMetric((float) (bnds[0].getX() + (bnds[1].getX() - bnds[0].getX()) / 2d),
						(float) (bnds[0].getY() + (bnds[1].getY() - bnds[0].getY()) / 2d),
						(float) (bnds[0].getZ() + (bnds[1].getZ() - bnds[0].getZ()) / 2d),
