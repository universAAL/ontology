...
public class Sphere extends Shape3D {
	...
	public float getDistanceTo(Shape shape) {
		...
		if (shape instanceof Sphere) {
			CoordinateSystem commonsys = CoordinateSystem.findCommonParentSystem(getLocalCoordinateSystem(),
					shape.getLocalCoordinateSystem());
			if (commonsys == null)
				throw new IllegalArgumentException("Not part of the same coordinate system chain");
			dist = Math.abs(((Sphere) shape).getCenter().getInHigherCoordinateSystem(commonsys)
					.getDistanceTo(getCenter().getInHigherCoordinateSystem(commonsys)));
	...
	public boolean contains(Shape3D shape) {
		...
		if (shape instanceof Box) {
			...
			double halfs[] = { ((Box) shape).getWidth() / 2d, ((Box) shape).getDepth() / 2d,
