...
public class LineSegment extends Line {
	...
	public float getDistanceTo(Point point) {
		...
		JgclPointOnCurve3D[] proj = (new JgclLine3D(new JgclCartesianPoint3D(a), new JgclCartesianPoint3D(b))
				.projectFrom(c));
	...
	private float distanceRay(Point lineStart, Point lineDir, Box box, float minDir, float maxDir) {
		...
		CoordinateSystem common = CoordinateSystem.findCommonParentSystem(lineStart.getCoordinateSystem(),
		...
		// be created
		Point invDir = new Point((float) (1f / lineDir.getX()), (float) (1f / lineDir.getY()),
				(float) (1f / lineDir.getZ()), new CoordinateSystem());
