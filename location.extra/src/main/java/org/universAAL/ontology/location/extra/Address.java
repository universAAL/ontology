...
/**
 * The class Address represents a generic address. This is an abstract class
 * because it is not possible to create a generic Address without any address
 * type (it is possible to do it using the PhysicalAddress and the MailBox
 * subclasses).
 * 
 * 
 * @author smazzei
...
public abstract class Address extends ManagedIndividual {
	...
	// it is possible to indicate up to two country names (sometimes it
	// could be useful to
	// indicate the country name in its original language and in a commonly
	// known language)
	...
	return new String[] { PROP_COUNTRY_NAME, PROP_EXTENDED_ADDRESS,
		PROP_LOCALITY, PROP_POSTAL_CODE, PROP_REGION };
    }
    ...
    /**
     * 
     * creates a new Address with URI and locality name. If URI is null an
    ...
    /**
     * @return the Address CountryNames. It can be a null object (if no country
     *         name is present), an array with one element (if only a country
     *         name is present), or an array with two elements (if two country
     *         names are present)
     */
     ...
     * 
     * @param countryName
     *            A String array with one or two elements (it is not possible to
     *            use a null value or an array with more than two elements)
     */
    ...
    /**
     * 
     * @return The Locality property (for a well formed Address this value is
     *         not null)
     */
