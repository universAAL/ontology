...
/**
 * The PhysicalAddress class represents an address that identifies a building
 * (i.e. with a street address)
 * 
 * @author smazzei
 * 
 */
public class PhysicalAddress extends Address {
	...
	// the building identifier is often a number (e.g. 280), but sometimes
	// it can be
	// an alphanumeric string (e.g. 41E)
	PROP_BUILDING_IDENTIFIER = uAAL_VOCABULARY_NAMESPACE
		+ "buildingIdentifier";
     ...
     * @param streetName
     *            A null value is not admitted for this argument
     * @param buildingIdentifier
     *            It can also be null
     */
    public PhysicalAddress(String uri, String locality, String streetName,
     ...
     *            It can also be null
     */
    public PhysicalAddress(String locality, String streetName,
	    String buildingIdentifier) {
	super(locality);
	setStreetAddress(streetName, buildingIdentifier);
    }
    ...
    /**
     * Get the street address (i.e. street name + building identifier) using a
     * streetAddress object. If the building identifier property is not set an
     * empty String is returned. If no street name is present a null object is
     * returned (in this case the object in not well formed)
     */
    public streetAddress getStreetAddress() {
	streetAddress addr = new streetAddress();
	if (props.containsKey(PROP_BUILDING_IDENTIFIER))
	    addr.buildingIdentifier = (String) props
		    .get(PROP_BUILDING_IDENTIFIER);
	else
