...
public class Questionnaire extends ManagedIndividual {
	public static final String MY_URI = QuestionnaireOntology.NAMESPACE + "Questionnaire";
	public static final String PROP_NAME = QuestionnaireOntology.NAMESPACE + "name";
	public static final String PROP_DESCRIPTION = QuestionnaireOntology.NAMESPACE + "description";
	public static final String PROP_HAS_QUESTION = QuestionnaireOntology.NAMESPACE + "hasQuestion";
	public static final String PROP_ORDERED_QUESTIONS = QuestionnaireOntology.NAMESPACE + "orderedQuestions";
	...
	public boolean isWellFormed() {
		return true && props.containsKey(PROP_NAME) && props.containsKey(PROP_DESCRIPTION)
				&& props.containsKey(PROP_HAS_QUESTION) && props.containsKey(PROP_ORDERED_QUESTIONS);
	}
	...
	public Question writeASingleChoiceQuestion(String qWording, Object correctAnswer, String answerType,
			ChoiceLabel[] choices) {
	...
	public Question writeAMultiChoiceQuestion(String qWording, Object[] correctAnswer, String[] answerType,
			ChoiceLabel[] choices) {
	...
	public Question writeAConditionalQuestion(String qWording, Object triggerAnswer, Question dependingQuestion,
			String answerType) {
	...
	public Question writeQuestionWithMedia(String qWording, String answerType, ChoiceLabel[] choices,
			MediaObject image) {
	...
	public Question writeQuestionWithMedia(String qWording, Object correctAnswer, String answerType,
	...
	public String questionnaireToString() {
		...
		for (int i = 0; i < questions.length; i++) {
			writtenQuestionnaire.append("Question " + index + ": " + questions[i].getQuestionWording() + "\n");
			...
			if (questions[i] instanceof ConditionalQuestion) {
				...
				if (ta.length == 1) {
					writtenQuestionnaire.append("Answer this question ONLY if you have answered " + "\""
							+ cq.getDependsOn().getLabel(ta[0]) + "\"" + " to question " + "\""
					...
					for (int k = 0; k < ta.length; k++) {
						writtenQuestionnaire.append("\"" + cq.getDependsOn().getLabel(ta[k]) + "\"");
						if (!(k == (ta.length - 1))) {
							writtenQuestionnaire.append(" and ");
						}
					}
					writtenQuestionnaire
							.append(" to question " + "\"" + cq.getDependsOn().getQuestionWording() + "\"" + "\n");
				}
			}
