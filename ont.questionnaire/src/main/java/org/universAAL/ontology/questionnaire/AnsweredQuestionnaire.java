...
public class AnsweredQuestionnaire extends ManagedIndividual {
	...
	public static final String MY_URI = QuestionnaireOntology.NAMESPACE + "AnsweredQuestionnaire";
	public static final String PROP_IS_ASSOCIATED_TO = QuestionnaireOntology.NAMESPACE + "isAssociatedTo";
	public static final String PROP_IS_COMPOSED_BY = QuestionnaireOntology.NAMESPACE + "isComposedBy";
	public static final String PROP_IS_ANSWERED_BY = QuestionnaireOntology.NAMESPACE + "isAnsweredBy";

	public static final String PROP_COMPLETENESS = QuestionnaireOntology.NAMESPACE + "completeness";
	...
	public boolean isWellFormed() {
		return true && props.containsKey(PROP_IS_ASSOCIATED_TO) && props.containsKey(PROP_IS_COMPOSED_BY)
				&& props.containsKey(PROP_IS_ANSWERED_BY) && props.containsKey(PROP_COMPLETENESS);
	...
	public double getCorrectAnswersPercentage() {
		...
		double correctPercentage = (numberOfCorrectQuestions / (numberOfAnsweredAnswers - numberOfNoCorrectAnswers))
				* 100;
	...
	public double getIncorrectAnswersPercentage() {
		...
		double incorrectPercentage = (numberOfIncorrectQuestions / (numberOfAnsweredAnswers - numberOfNoCorrectAnswers))
				* 100;
	...
	public Answer[] getIncorrectAnswers() {
		...
		for (int i = 0; i < answers.length; i++) {
			Question question = answers[i].getAssociatedQuestion();
			if (question.isHasCorrectAnswer() && !(answers[i].isCorrectAnswer() == Answer.CORRECT_ANSWER)) {
			...
			if (answerMap.containsKey(allQuestions[i])) { // if the question has
				...
				if (allQuestions[i] instanceof ConditionalQuestion) { // if the
					...
					// to which is related to, haven't appeared
							(cq.getTriggerAnswer() != ((Answer) answerMap.get(depends)).getAnswerContent())) // or
						// the
						// depending
						// question
						// has
