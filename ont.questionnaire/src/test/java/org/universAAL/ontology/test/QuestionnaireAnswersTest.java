...
public class QuestionnaireAnswersTest extends OntTestCase {
	...
	protected void setUp() throws Exception {
		...
		q1 = new SingleChoiceQuestion("Are vegetables good for improving your health condition?", Boolean.TRUE,
				TypeMapper.getDatatypeURI(Boolean.class), choices2);
		q2 = new SingleChoiceQuestion("Would you answer one more question?", TypeMapper.getDatatypeURI(Boolean.class),
				choices2);
		q3 = new ConditionalQuestion("When did your caregiver recommended you to practice your relaxation exercices?",
		...
		q4 = new MultiChoiceQuestion("When do you have to take your pills?", correctAnswers, correctAnswersURI,
				choices1);
		q5 = new SingleChoiceQuestion("When do you prefer to exercise?", TypeMapper.getDatatypeURI(PartsOfDay.class),
				choices1);
		q6 = new ConditionalQuestion("Do you usually practice sport at this time?", PartsOfDay.IN_THE_AFTERNOON, q5,
				TypeMapper.getDatatypeURI(Boolean.class), choices2);
		q7 = new SingleChoiceQuestion("When are you exercising tomorrow?", PartsOfDay.IN_THE_MORNING,
	 ...
	 * answerer right the same number of questions that we have answered wrong.
	 */
	...
	public void testXQuestionGetter() {
		...
		ArrayList<Question> incorrectQuestions = new ArrayList<Question>(
				Arrays.asList(aq.getAssociatedQuestions(aq.getIncorrectAnswers())));
		ArrayList<Question> correctQuestions = new ArrayList<Question>(
				Arrays.asList(aq.getAssociatedQuestions(aq.getCorrectAnswers())));
		ArrayList<Question> noCorrectQuestions = new ArrayList<Question>(
				Arrays.asList(aq.getAssociatedQuestions(aq.getNoCorrectAnswers())));

		assertTrue(incorrectQuestions.contains(q3) && incorrectQuestions.contains(q4) && incorrectQuestions.contains(q7)
				&& incorrectQuestions.size() == 3);
		assertTrue(correctQuestions.contains(q1) && correctQuestions.size() == 1);
		assertTrue(noCorrectQuestions.contains(q2) && noCorrectQuestions.contains(q5) && noCorrectQuestions.contains(q6)
