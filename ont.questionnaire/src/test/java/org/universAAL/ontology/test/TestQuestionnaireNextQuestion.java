...
/**
 * This test checks the method nextQuestion, when the given questionnaire
 * has the questions ordered or out of order.
 * @author mdelafuente
 *
 */
...
public class TestQuestionnaireNextQuestion extends IntegrationTest{
	...
	@Test
	public void simulatePath(AnsweredQuestionnaire aq, Map <Question, Object> userAnswers){
		AnsweredQuestionnaire aqPathX = aq;
		Question q = aqPathX.nextQuestion();
		Question previousQuestion = null;
		ArrayList<Question> questions = new ArrayList<Question>(Arrays.asList(aqPathX.getAssociatedQuestionnaire().getQuestions()));

		while(q!=null){

			if(! (aqPathX.getAssociatedQuestionnaire().isOrderedQuestions())){ //if the question has NOT ordered questions
				...
				assertTrue(userAnswers.containsKey(q) && (questions.indexOf(q)>=questions.indexOf(previousQuestion))  );
				aqPathX.answeredToQuestion(q,userAnswers.get(q));
				previousQuestion = q;
				q=aqPathX.nextQuestion(); 
			}
	...
	public void testPath1(){
		AnsweredQuestionnaire aqPath1 = new AnsweredQuestionnaire(questionnaire, peter);
	...
	public void testPath2(){
		AnsweredQuestionnaire aqPath2 = new AnsweredQuestionnaire(questionnaire, peter);
		Map <Question,Object> userAnswers = new HashMap<Question,Object>();
	...
	public void testPath1NotOrdered(){
		...
		notOrderedQuestionnaire.setDescription("A questionnaire with not ordered questions");
		notOrderedQuestionnaire.setOrderedQuestions(false);
		AnsweredQuestionnaire aqPath1NO = new AnsweredQuestionnaire(notOrderedQuestionnaire, peter);
		Map <Question,Object> userAnswers = new HashMap<Question,Object>();
		userAnswers.put(q1,Boolean.FALSE);
		userAnswers.put(q2, PartsOfDay.IN_THE_AFTERNOON);
		userAnswers.put(q3,Boolean.FALSE);
