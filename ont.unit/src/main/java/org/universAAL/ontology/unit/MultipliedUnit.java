...
public class MultipliedUnit extends DerivedUnit {
    public static final String MY_URI = UnitOntology.NAMESPACE
	    + "MultipliedUnit";
    public static final String PROP_MULTIPLIED_BY = UnitOntology.NAMESPACE
	    + "multipliedBy";
    public static final String PROP_MULTIPLIED_BY_PREFIX = UnitOntology.NAMESPACE
	    + "multipliedByPrefix";
	...
	if (!hasProperty(PROP_SYMBOL)) {
		...
		mult = getMultipliedByPrefix().getSymbol()
			+ getMultipliedBy().getSymbol();
	    } else {
