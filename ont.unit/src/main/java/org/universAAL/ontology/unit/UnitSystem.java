...
public class UnitSystem extends ManagedIndividual {
	public static final String MY_URI = UnitOntology.NAMESPACE
			+ "UnitSystem";
	public static final String PROP_UNITS = UnitOntology.NAMESPACE
			+ "units";
	...
	public Unit[] getUnits() {
		...
		if (propList instanceof List)
			...
			return new Unit[] {(Unit)propList}; // Handle special case of a single item not contained in a list
		return new Unit[0];
	}
	...
	public Prefix[] getPrefixes() {
		...
		if (propList instanceof List)
			...
			return new Prefix[] {(Prefix)propList}; // Handle special case of a single item not contained in a list
		return new Prefix[0];
	}
