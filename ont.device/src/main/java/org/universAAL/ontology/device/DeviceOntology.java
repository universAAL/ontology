...
public final class DeviceOntology extends Ontology {
	...
	public void create() {
		...
		oci_EnvironmentMeter.addObjectProperty(EnvironmentMeter.PROP_HAS_VALUE)
				.setFunctional();
		oci_EnvironmentMeter.addRestriction(MergedRestriction
				.getAllValuesRestrictionWithCardinality(
						EnvironmentMeter.PROP_HAS_VALUE, Measurement.MY_URI, 1,
						1));
		...
		oci_DoorActuator.addRestriction(MergedRestriction
				.getAllValuesRestrictionWithCardinality(
						DoorActuator.PROP_HAS_VALUE, StatusValue.MY_URI, 1, 1));
