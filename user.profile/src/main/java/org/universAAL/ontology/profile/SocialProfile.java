...
public class SocialProfile extends ManagedIndividual implements PropertyPublisher {
	...
	public static String[] getStandardPropertyURIs() {
		return new String[] {
				PROP_S_HOUSE_WORKER_ID,
				PROP_S_LIVE_ALONE,
				PROP_S_MAIN_CONTACT_ID,
				PROP_S_PHYCHO_STATE
		};
	}
	...
	public ProfileProperty[] getStaticProperties() {
		...
		ppArray[0] = new ProfileProperty((getHouseWorkerId()), PROP_S_HOUSE_WORKER_ID, "House worker ID", true);
		ppArray[1] = new ProfileProperty((getLiveAlone()), PROP_S_LIVE_ALONE, "Live alone?", true);
		ppArray[2] = new ProfileProperty((getsMainContactId()), PROP_S_MAIN_CONTACT_ID, "Main contact ID", true);
		ppArray[3] = new ProfileProperty(
				((getPhychoState() == null) ? "" : getPhychoState()), PROP_S_PHYCHO_STATE, "Phycho state", true);
