...
public class ElderlyProfile extends UserProfile {
	...
	static {
		...
		PROP_DIARY_ACTIVITY_PROFILE = PROFILING_NAMESPACE + "diaryActionProfile";
	...
	public static Restriction getClassRestrictionsOnProperty(String propURI) {
		if (PROP_SOCIAL_PROFILE.equals(propURI))
			return Restriction.getAllValuesRestrictionWithCardinality(propURI, SocialProfile.MY_URI, 1, 1);
		if (PROP_HEALTH_PROFILE.equals(propURI))
			return Restriction.getAllValuesRestrictionWithCardinality(propURI, HealthProfile.MY_URI, 1, 1);
		if (PROP_DIARY_ACTIVITY_PROFILE.equals(propURI))return Restriction.getAllValuesRestrictionWithCardinality(propURI,
				DiaryActivityProfile.MY_URI, 1, 1); 
		if (PROP_DIARY_NUTR_PROFILE.equals(propURI))return Restriction.getAllValuesRestrictionWithCardinality(propURI,
				DiaryNutritionalProfile.MY_URI, 1, 1); 
		if (PROP_PERS_PREF_PROFILE.equals(propURI))return Restriction.getAllValuesRestrictionWithCardinality(propURI,
	...
	public static ElderlyProfile loadInstance() {
		...
		eProfile.setDiaryNutritionalProfile(DiaryNutritionalProfile.loadInstance());
		eProfile.setDiaryActivityProfile(DiaryActivityProfile.loadInstance());
		eProfile.setPersonalPreferenceProfile(PersonalPreferenceProfile.loadInstance());
	...
	public void setProperty(String propURI, Object o) {
		...
		else if (PROP_HEALTH_PROFILE.equals(propURI) && o instanceof HealthProfile)
			setHealthProfile((HealthProfile)o);
		else if (PROP_DIARY_ACTIVITY_PROFILE.equals(propURI) && o instanceof DiaryActivityProfile)
			setDiaryActivityProfile((DiaryActivityProfile)o);
		else if (PROP_DIARY_NUTR_PROFILE.equals(propURI) && o instanceof DiaryNutritionalProfile)
			setDiaryNutritionalProfile((DiaryNutritionalProfile)o);
		else if (PROP_PERS_PREF_PROFILE.equals(propURI) && o instanceof PersonalPreferenceProfile)
	...
	public ProfileProperty[] getDynamicProperties() {
		...
		allProperties[i++] = new ProfileProperty(new SocialProfile(), PROP_SOCIAL_PROFILE, "Social Profile", false);
		allProperties[i++] = new ProfileProperty(new HealthProfile(), PROP_HEALTH_PROFILE, "Health Profile", false);
		allProperties[i++] = new ProfileProperty(new DiaryActivityProfile(), PROP_DIARY_ACTIVITY_PROFILE, "Diary Activity Profile", false);
		allProperties[i++] = new ProfileProperty(new DiaryNutritionalProfile(), PROP_DIARY_NUTR_PROFILE, "Diary Nutritional Profile", false);
	...
	public ProfileProperty[] getStaticProperties() {
		...
		allProperties[i++] = new ProfileProperty(
				((getSocialProfile() == null) ? new SocialProfile() : getSocialProfile()), 
				PROP_SOCIAL_PROFILE, "Social Profile", true);
		allProperties[i++] = new ProfileProperty(
				((getHealthProfile() == null) ? new HealthProfile() : getHealthProfile()), 
				PROP_HEALTH_PROFILE, "Health Profile", true);
		allProperties[i++] = new ProfileProperty(
				((getDiaryActivityProfile() == null) ? new DiaryActivityProfile() : getDiaryActivityProfile()), 
				PROP_DIARY_ACTIVITY_PROFILE, "Diary Activity Profile", true);
		allProperties[i++] = new ProfileProperty(
				((getDiaryNutritionalProfile() == null) ? new DiaryNutritionalProfile() : getDiaryNutritionalProfile()), 
				PROP_DIARY_NUTR_PROFILE, "Diary Nutritional Profile", true);
		allProperties[i++] = new ProfileProperty(
				((getPersonalPreferenceProfile() == null) ? new PersonalPreferenceProfile() : getPersonalPreferenceProfile()), 
