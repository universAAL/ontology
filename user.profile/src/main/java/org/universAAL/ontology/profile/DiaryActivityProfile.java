...
public class DiaryActivityProfile extends ManagedIndividual implements PropertyPublisher {
	...
	public static Restriction getClassRestrictionsOnProperty(String propURI) {
		if (PROP_S_EXERCISE.equals(propURI))
			return Restriction.getAllValuesRestrictionWithCardinality(propURI, ExerciseType.MY_URI, 1, 1);
	...
	public void setProperty(String propURI, Object o) {
		...
		else if (PROP_S_BREAKFAST_TIME.equals(propURI) && o instanceof XMLGregorianCalendar)
			setBreakfastTime((XMLGregorianCalendar)o);
		else if (PROP_S_LAUNCH_TIME.equals(propURI) && o instanceof XMLGregorianCalendar)
			setLaunchTime((XMLGregorianCalendar)o);
		else if (PROP_S_DINNER_TIME.equals(propURI) && o instanceof XMLGregorianCalendar)
			setDinnerTime((XMLGregorianCalendar)o);
		else if (PROP_D_BED_TIME.equals(propURI) && o instanceof XMLGregorianCalendar)
		...
		else if (PROP_D_GET_UP.equals(propURI) && o instanceof XMLGregorianCalendar)
	...
	public static String[] getStandardPropertyURIs() {
		return new String[] {
				PROP_S_EXERCISE,
				PROP_S_BREAKFAST_TIME,
				PROP_S_LAUNCH_TIME,
				PROP_S_DINNER_TIME,
				PROP_S_RECOM_ACTIVITY,
				PROP_D_BED_TIME,
				PROP_D_DIARY_ACTIVITIES,
				PROP_D_GET_UP,
		};
	}
	...
	public ProfileProperty[] getDynamicProperties() {
		...
		propArray[0] = new ProfileProperty(
				((getGetUp() == null) ? TypeMapper.getDataTypeFactory().newXMLGregorianCalendar() : getGetUp()), 
				PROP_D_GET_UP, "Get up time", false);
		propArray[1] = new ProfileProperty(
				((getBedTime() == null) ? TypeMapper.getDataTypeFactory().newXMLGregorianCalendar() : getBedTime()), 
				PROP_D_BED_TIME, "Bed time", false);
		propArray[2] = new ProfileProperty(
				((getDiaryActivities() == null) ? "" : getDiaryActivities()), 
				PROP_D_DIARY_ACTIVITIES, "Dairy Activities", false);
	...
	public ProfileProperty[] getStaticProperties() {
		...
		propArray[0] = new ProfileProperty(
				((getExerciseType() == null) ? ExerciseType.getExerciseTypeByOrder(0) : getExerciseType()), 
				PROP_S_EXERCISE, "Exercise Type", true);
		propArray[1] = new ProfileProperty(
				((getBreakfastTime() == null) ? TypeMapper.getDataTypeFactory().newXMLGregorianCalendar() : getBreakfastTime()), 
				PROP_S_BREAKFAST_TIME, "Breakfast time", true);
		propArray[2] = new ProfileProperty(
				((getLaunchTime() == null) ? TypeMapper.getDataTypeFactory().newXMLGregorianCalendar() : getLaunchTime()), 
				PROP_S_LAUNCH_TIME, "Launch time", true);
		propArray[3] = new ProfileProperty(
				((getDinnerTime() == null) ? TypeMapper.getDataTypeFactory().newXMLGregorianCalendar() : getDinnerTime()), 
				PROP_S_DINNER_TIME, "Dinner time", true);
		propArray[4] = new ProfileProperty(
				((getRecommendedActivity() == null) ? "" : getRecommendedActivity()), 
				PROP_S_RECOM_ACTIVITY, "Recommended Activity", true);
