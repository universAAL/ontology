...
public class UserIdentificationProfile extends ManagedIndividual implements PropertyPublisher {
	...
	public static Restriction getClassRestrictionsOnProperty(String propURI) {
		if (propURI == null)
			return null;
		//Should we use Cardinality(MAX,MIN)= (1,1) for the required single-value properties such as a compulsory field in a XFORM and (1,0) for optional?
		//or Semantically the property must exist so Cardinality(MAX,MIN) = (1,1)  and the property may be just null value in reality
		...
		if (PROP_S_USER_ROLE.equals(propURI))
			return Restriction.getAllValuesRestrictionWithCardinality(propURI, TypeMapper.getDatatypeURI(UserRole.class),1,1);
		if (PROP_S_GENDER.equals(propURI))
			return Restriction.getAllValuesRestrictionWithCardinality(propURI, TypeMapper.getDatatypeURI(Sex.class),1,1);
		if (PROP_S_ADDRESS.equals(propURI))
			return Restriction.getAllValuesRestrictionWithCardinality(propURI, Address.MY_URI,1,1);
	...
	public void setProperty(String propURI, Object o) {
		if (PROP_S_USER_ROLE_ID.equals(propURI)  &&  o instanceof Integer)
			...
			setSurname((String)o);
		else if (PROP_S_PREFERRED_LANGUAGE.equals(propURI)  &&  o instanceof String)
	...
	public static String[] getStandardPropertyURIs() {
		return new String[] {
				PROP_S_AGE,
				PROP_S_EMAIL,
				PROP_S_HOME_TELEPHONE,
				PROP_S_MOBILE_TELEPHONE,
				PROP_S_NAME,
				PROP_S_NATIONALITY,
				PROP_D_USER_IP,
				PROP_S_OFFICE_TELEPHONE,
				PROP_S_PREFERRED_LANGUAGE,
				PROP_S_GENDER,
				PROP_S_SURNAME,
				PROP_S_USER_ID,
				PROP_S_USER_ROLE_ID,
	...
	public static String RDFSComment(){return "The class of the User Identification Profile";}
	
	public static String getRDFSLabel(){return "User Identification Profile";}
...
//		return -1;
	...
	public ProfileProperty[] getDynamicProperties() {
		...
		propArray[0] = new ProfileProperty(
				((getUserIP() == null) ? "" : getUserIP()), PROP_D_USER_IP, "User IP Address", false);
	...
	public ProfileProperty[] getStaticProperties() {
		...
		propArray[0] = new ProfileProperty(getUserID() , PROP_S_USER_ID, "User ID", true);
		propArray[1] = new ProfileProperty(
				((getName() == null) ? "" : getName()), PROP_S_NAME, "Name", true);
		propArray[2] = new ProfileProperty(
				((getSurname() == null) ? "" : getSurname()), PROP_S_SURNAME, "Surname", true);
		propArray[3] = new ProfileProperty(getAge(), PROP_S_AGE, "Age", true);
		propArray[4] = new ProfileProperty(
				((getEmail() == null) ? "" : getEmail()), PROP_S_EMAIL, "E-mail", true);
		propArray[5] = new ProfileProperty(
				((getHomeTelephone() == null) ? "" : getHomeTelephone()), PROP_S_HOME_TELEPHONE, "Home telephone", true);
		propArray[6] = new ProfileProperty(
				((getOfficeTelephone() == null) ? "" : getOfficeTelephone()), PROP_S_OFFICE_TELEPHONE, "Office telephone", true);
		propArray[7] = new ProfileProperty(
				((getMobileTelephone() == null) ? "" : getMobileTelephone()), PROP_S_MOBILE_TELEPHONE, "Mobile telephone", true);
//		propArray[8] = new ProfileProperty(
//				((getGender() == null) ? Sex.getGenderByOrder(0) : getGender()), PROP_S_GENDER, "Gneder", true);
		propArray[8] = new ProfileProperty(Sex.female, PROP_S_GENDER, "Gender", true);
		propArray[9] = new ProfileProperty(
				((getNationality() == null) ? "" : getNationality()), PROP_S_NATIONALITY, "Nationality", true);
		propArray[10] = new ProfileProperty(
				((getPreferredLanguage() == null) ? "" : getPreferredLanguage()), PROP_S_PREFERRED_LANGUAGE, "Preferred language", true);
		propArray[11] = new ProfileProperty(getUserRoleID(), PROP_S_USER_ROLE_ID, "User role ID", true);
		propArray[12] = new ProfileProperty(
				((getUserRole() == null) ? UserRole.getUserRoleByOrder(0) : getUserRole()), 
				PROP_S_USER_ROLE, "User role", true);
