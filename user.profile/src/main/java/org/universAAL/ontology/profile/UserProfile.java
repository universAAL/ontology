...
public class UserProfile extends Profile {
	...
	{
		if(PROP_USER_ID_PROFILE.equals(propURI)) return Restriction.getAllValuesRestrictionWithCardinality(propURI, UserIdentificationProfile.MY_URI,1,1);

		return Profile.getClassRestrictionsOnProperty(propURI); 
	}
	
	public void setProperty(String propURI, Object o) {
		if (PROP_USER_ID_PROFILE.equals(propURI)  &&  o instanceof UserIdentificationProfile)
	...
	public void setUserIdentificationProfile(UserIdentificationProfile userIdProfile){
		if(userIdProfile!=null) 
			props.put (PROP_USER_ID_PROFILE, userIdProfile);
...
//Later must be modified; properties (contained key) must be returned
	...
	public ProfileProperty[] getDynamicProperties() {
		...
		propArray[i++] = new ProfileProperty(
				((getUserIdentificationProfile() == null) ? new UserIdentificationProfile() : getUserIdentificationProfile()),
	...
	public static UserProfile loadInstace() {
		...
		up.setUserIdentificationProfile(UserIdentificationProfile.loadInstance());
		return up;
	}
}