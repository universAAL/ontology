...
public abstract class Treatment extends ManagedIndividual {
	...
	public static final String MY_URI = HealthProfileOntology.NAMESPACE + "Treatment";
	public static final String PROP_NAME = HealthProfileOntology.NAMESPACE + "name";
	public static final String PROP_COMPLETENESS = HealthProfileOntology.NAMESPACE + "completeness";
	public static final String PROP_STATUS = HealthProfileOntology.NAMESPACE + "status";

	public static final String PROP_HAS_TREATMENT_PLANNING = HealthProfileOntology.NAMESPACE + "hasTreatmentPlanning";
	public static final String PROP_IS_PRESCRIBED_BY_CAREGIVER = HealthProfileOntology.NAMESPACE + "hasCaregiver";
	public static final String PROP_MOTIVATIONAL_STATUS = HealthProfileOntology.NAMESPACE + "motivationalStatus";
	public static final String PROP_HAS_PRIVACY = HealthProfileOntology.NAMESPACE + "hasPrivacy";
	public static final String PROP_DESCRIPTION = HealthProfileOntology.NAMESPACE + "description";
	public static final String PROP_HAS_PERFORMED_SESSION = HealthProfileOntology.NAMESPACE + "hasPerformedSession";

	public static final String PROP_IS_ASSOCIATED_TO_DISEASE = HealthProfileOntology.NAMESPACE
	...
	public Treatment(AssistedPersonProfile assistedPerson, Caregiver caregiver, String tname, String description,
			XMLGregorianCalendar stDt, String diseaseURI) {
		// super(assistedPerson + "." + name);
		this.setName(tname);
		this.setCaregiver(caregiver);
		this.setCompleteness(0);
		this.setMotivationalStatus(MotivationalStatusType.precontemplation); // until
		// the
		// treatment
		// is
		// not
		...
		// precontemplation
		// status
		this.checkStatus(stDt); // depending on the date, the treatment will be
		// actived or planned
		this.setAssociatedDisease(diseaseURI);
	}

	public Treatment(AssistedPersonProfile assistedPerson, Caregiver caregiver, String tname, TreatmentPlanning tp,
		...
		// precontemplation
		// status
		this.checkStatus(tp.getStartDate()); // depending on the date, the
		// treatment will be actived or
		// planned
		this.setAssociatedDisease(diseaseURI);
	}
	...
	public Treatment(String tname, String description, String diseaseURI) {
		...
		// precontemplation
		// status
		this.checkStatus(); // depending on the date, the treatment will be
		// actived or planned
		this.setAssociatedDisease(diseaseURI);
	}
	...
	public boolean isWellFormed() {
		return true && props.containsKey(PROP_NAME) && props.containsKey(PROP_COMPLETENESS)
				&& props.containsKey(PROP_STATUS) && props.containsKey(PROP_HAS_TREATMENT_PLANNING)
				&& props.containsKey(PROP_IS_PRESCRIBED_BY_CAREGIVER) && props.containsKey(PROP_MOTIVATIONAL_STATUS)
				&& props.containsKey(PROP_HAS_PRIVACY) && props.containsKey(PROP_DESCRIPTION)
				&& props.containsKey(PROP_HAS_PERFORMED_SESSION) && props.containsKey(PROP_IS_ASSOCIATED_TO_DISEASE);
	}
	...
	public PerformedSession[] getPerformedSessions() {
		...
		if (propList instanceof List)
			...
			return new PerformedSession[] { (PerformedSession) propList }; // Handle
		// special
		// case
		// of
		// a
	...
	public void checkStatus(XMLGregorianCalendar stDate) {
		...
		try {
			...
			if (currentDate.compare(stDate) == DatatypeConstants.GREATER
					|| currentDate.compare(stDate) == DatatypeConstants.EQUAL)
	...
	public void checkStatus() {
		this.setStatus(StatusType.actived);
		// si no se proporciona una fecha de inicio se toma la actual como
		// inicio
