	...
	public void create() {
		...
		oci_HealthMeasurement.addRestriction(MergedRestriction
				.getAllValuesRestrictionWithCardinality(
						HealthMeasurement.PROP_OBTAINED_BY, User.MY_URI, 1, 1));
		...
		oci_HealthMeasurement.addRestriction(MergedRestriction
				.getAllValuesRestrictionWithCardinality(
						HealthMeasurement.PROP_MEASURED_FROM, Device.MY_URI, 1,
						1));

		oci_HealthMeasurement.addDatatypeProperty(HealthMeasurement.PROP_NAME);
		oci_HealthMeasurement.addRestriction(MergedRestriction
				.getAllValuesRestrictionWithCardinality(
						HealthMeasurement.PROP_NAME,
		...
		oci_PersonWeight.addRestriction(MergedRestriction
				.getAllValuesRestrictionWithCardinality(Measurement.PROP_VALUE,
						TypeMapper.getDatatypeURI(Float.class), 1, 1));

		oci_PersonWeight.addRestriction(MergedRestriction
				.getFixedValueRestriction(Measurement.PROP_HAS_PREFIX,
						InternationalSystem.IND_PREFIX_SI_KILO));
		oci_PersonWeight.addRestriction(MergedRestriction
				.getFixedValueRestriction(Measurement.PROP_HAS_UNIT,
		...
		unitR.appendTo(diasR, new String[] { BloodPressure.PROP_DIASTOLIC,
				Measurement.PROP_HAS_UNIT });
		typeR.appendTo(diasR, new String[] { BloodPressure.PROP_DIASTOLIC,
				Measurement.PROP_VALUE });

		unitR.appendTo(sysR, new String[] { BloodPressure.PROP_SYSTOLIC,
				Measurement.PROP_HAS_UNIT });
		typeR.appendTo(sysR, new String[] { BloodPressure.PROP_SYSTOLIC,
				Measurement.PROP_VALUE });
		...
		oci_HeartRate.addRestriction(MergedRestriction
				.getFixedValueRestriction(Measurement.PROP_HAS_UNIT,
						beatsPerMinute));
		...
		oci_BloodOxygenSat.addRestriction(MergedRestriction
				.getFixedValueRestriction(Measurement.PROP_HAS_UNIT, spo2));
