...
public final class HealthMeasurementOntology extends Ontology {
	...
	public void create() {
		...
		OntClassInfoSetup oci_BloodOxygenSat = createNewOntClassInfo(BloodOxygenSaturation.MY_URI, factory, 9);
		...
		oci_HealthMeasurement.addObjectProperty(HealthMeasurement.PROP_OBTAINED_BY).setFunctional();
		oci_HealthMeasurement.addRestriction(MergedRestriction
				.getAllValuesRestrictionWithCardinality(HealthMeasurement.PROP_OBTAINED_BY, User.MY_URI, 1, 1));

		oci_HealthMeasurement.addObjectProperty(HealthMeasurement.PROP_MEASURED_FROM).setFunctional();
		// oci_HealthMeasurement.addRestriction(MergedRestriction.getCardinalityRestriction(HealthMeasurement.PROP_MEASURED_BY,
		// 1, 1));
		oci_HealthMeasurement.addRestriction(MergedRestriction
				.getAllValuesRestrictionWithCardinality(HealthMeasurement.PROP_MEASURED_FROM, Device.MY_URI, 1, 1));

		oci_HealthMeasurement.addDatatypeProperty(HealthMeasurement.PROP_NAME);
		oci_HealthMeasurement.addRestriction(MergedRestriction.getAllValuesRestrictionWithCardinality(
				HealthMeasurement.PROP_NAME, TypeMapper.getDatatypeURI(String.class), 1, 1));

		oci_HealthMeasurement.addDatatypeProperty(HealthMeasurement.PROP_DESCIPTION);
		oci_HealthMeasurement.addRestriction(MergedRestriction.getAllValuesRestrictionWithCardinality(
		...
		oci_PersonWeight.addRestriction(MergedRestriction.getAllValuesRestrictionWithCardinality(Measurement.PROP_VALUE,
				TypeMapper.getDatatypeURI(Float.class), 1, 1));

		oci_PersonWeight.addRestriction(MergedRestriction.getFixedValueRestriction(Measurement.PROP_HAS_PREFIX,
				InternationalSystem.IND_PREFIX_SI_KILO));
		oci_PersonWeight.addRestriction(MergedRestriction.getFixedValueRestriction(Measurement.PROP_HAS_UNIT,
		...
		Unit mmHg = new Unit("mmHg", "mm Hg", "mm Hg", MeasurableDimension.Derived);

		MergedRestriction diasR = MergedRestriction.getAllValuesRestrictionWithCardinality(BloodPressure.PROP_DIASTOLIC,
				Measurement.MY_URI, 1, 1);
		MergedRestriction sysR = MergedRestriction.getAllValuesRestrictionWithCardinality(BloodPressure.PROP_SYSTOLIC,
				Measurement.MY_URI, 1, 1);
		MergedRestriction unitR = MergedRestriction.getFixedValueRestriction(Measurement.PROP_HAS_UNIT, mmHg);
		MergedRestriction typeR = MergedRestriction.getAllValuesRestriction(Measurement.PROP_VALUE,
		...
		Unit beatsPerMinute = new DividedUnit("BPM", Util.IND_UNIT_UNITY, TimeSystem.IND_UNIT_TS_MINUTE);
		oci_HeartRate
				.addRestriction(MergedRestriction.getFixedValueRestriction(Measurement.PROP_HAS_UNIT, beatsPerMinute));
		...
		Unit spo2 = new Unit("spo2", "SpO2", "%SpO2", MeasurableDimension.Adiemnsional);
		oci_BloodOxygenSat.addRestriction(MergedRestriction.getFixedValueRestriction(Measurement.PROP_HAS_UNIT, spo2));
		...
		// XXX: set restriction so that the unit if the signal is the same as
		// HeartRate.
