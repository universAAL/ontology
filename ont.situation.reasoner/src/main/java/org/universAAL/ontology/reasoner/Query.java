...
public class Query extends Persistent {
	...
	public static final String PROP_FULL_TEXT_QUERY = ReasoningOntology.NAMESPACE + "fullQuery";
	public static final String PROP_SEARCH_STRING = ReasoningOntology.NAMESPACE + "searchString";
	public static final String PROP_RESULTING_EVENT = ReasoningOntology.NAMESPACE + "resultingEvent";
	...
	public boolean equals(Query query) {
		...
		return query.getSearchString().equals(this.getSearchString())
				&& (event1.getRDFSubject() == event2.getRDFSubject()
						|| event1.getRDFSubject().equals(event2.getRDFSubject()))
				&& (event1.getRDFPredicate() == event2.getRDFPredicate()
						|| event1.getRDFPredicate().equals(event2.getRDFPredicate()))
				&& (event1.getRDFObject() == event2.getRDFObject()
						|| event1.getRDFObject().equals(event2.getRDFObject()));
	}
	...
	private String getResultingQuery(String lineBreak) {
		...
		query.append("<").append(getResultingEvent().getRDFSubject().getURI()).append("> ");
		query.append("<").append(Resource.PROP_RDF_TYPE).append("> ");
		query.append("<").append(getResultingEvent().getSubjectTypeURI()).append(">");
		query.append(". }").append(lineBreak);
	...
	public String toString() {
		...
		return "<html>" + getURI() + ":<br>"
				+ query.replace("<", "&lt;").replace(">", "&gt;").replace(System.getProperty("line.separator"), "<br>")
				+ "</html>";
	}
}
