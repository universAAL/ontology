...
public final class SecurityOntology extends Ontology {
	...
	public void create() {
		...
		// OntClassInfoSetup oci_credentials =
		// createNewAbstractOntClassInfo(Credentials.MY_URI);
		...
		oci_KnowledgeF.setResourceComment(
				"the user is required to prove knowledge of a secret in order to authenticate, for example a password.");
		...
		oci_PossessionF.setResourceComment(
		...
		oci_InherenceF
				.setResourceComment("These are factors associated with the user, and are usually biometric methods,"
		...
		// OntClassInfoSetup oci_anonymizable =
		// createNewAbstractOntClassInfo(Anonymizable.MY_URI);
		...
		oci_anonymizable.setResourceComment(
				"Any anonymizable resource should be subclass of this class to enable the anonymization service to encrypt the property reference");
		oci_anonymizable.addObjectProperty(Anonymizable.PROP_ANNONYMOUS_RESOURCE).setFunctional();

		/*
		 * Extension of profiling ontology to automatically allow for
		 * subprofiles to be anonymizable
		 */
		...
		OntClassInfoSetup oci_multFactorCred = createNewOntClassInfo(MultifactorCredentials.MY_URI, factory, 6);
		OntClassInfoSetup oci_passwd = createNewOntClassInfo(Password.MY_URI, factory, 7);
		OntClassInfoSetup oci_otp = createNewOntClassInfo(OneTimePassword.MY_URI, factory, 8);
		...
		OntClassInfoSetup oci_roleMngService = createNewOntClassInfo(RoleManagementService.MY_URI, factory, 13);
		...
		oci_multFactorCred.setResourceComment(
				"Authentication using more than one factor to achieve more confidence about the identity of the user");
		oci_multFactorCred.setResourceLabel("MultiFactorCredentials");
		oci_multFactorCred.addSuperClass(Credentials.MY_URI);

		oci_multFactorCred.addDatatypeProperty(MultifactorCredentials.PROP_FACTORS).setFunctional();
		oci_multFactorCred.addObjectProperty(MultifactorCredentials.PROP_FACTORS);
		oci_multFactorCred.addRestriction(
				MergedRestriction.getAllValuesRestriction(MultifactorCredentials.PROP_FACTORS, Factor.MY_URI));

		// Password
		oci_passwd.setResourceComment(
				"Password factor. A password the user knows to be used in conjuntion with other factors to authenticate the user");
		...
		oci_passwd.addRestriction(MergedRestriction.getAllValuesRestrictionWithCardinality(Password.PROP_PASSWORD,
				TypeMapper.getDatatypeURI(Base64Binary.class), 1, 1));

		oci_passwd.addObjectProperty(Password.PROP_PASSWORD_DIGEST).setFunctional();
		oci_passwd.addRestriction(MergedRestriction
				.getAllValuesRestrictionWithCardinality(Password.PROP_PASSWORD_DIGEST, Digest.MY_URI, 0, 1));

		// OneTimePassword
		oci_otp.setResourceComment(
		...
		oci_userPass.addRestriction(MergedRestriction.getAllValuesRestrictionWithCardinality(
				UserPasswordCredentials.PROP_USERNAME, TypeMapper.getDatatypeURI(String.class), 1, 1));
		...
		oci_asset.setResourceComment(
				"Mark a class or an instance as a security Asset to be access through the authorisation service");
		oci_asset.addObjectProperty(Asset.PROP_HAS_DEFAULT_ACCESS);
		oci_asset.addRestriction(MergedRestriction.getAllValuesRestrictionWithCardinality(Asset.PROP_HAS_DEFAULT_ACCESS,
				AccessRight.MY_URI, 0, 1));
		...
		oci_role.addRestriction(
				MergedRestriction.getAllValuesRestriction(Role.PROP_HAS_ACCESS_RIGHTS, AccessRight.MY_URI));
		...
		oci_accessType.toEnumeration(
				new ManagedIndividual[] { AccessType.read, AccessType.change, AccessType.add, AccessType.remove });

		// Access Right
		oci_AccessRight.addSuperClass(ManagedIndividual.MY_URI);
		oci_AccessRight.setResourceLabel("Access Right");
		oci_AccessRight.setResourceComment(
				"Access Right describes the type of access right to a specific (or generic) class (or instance) of Asset(s).");
		oci_AccessRight.addObjectProperty(AccessRight.PROP_ACCESS_TYPE);
		oci_AccessRight.addRestriction(MergedRestriction
				.getAllValuesRestrictionWithCardinality(AccessRight.PROP_ACCESS_TYPE, AccessType.MY_URI, 1, 4));
		oci_AccessRight.addObjectProperty(AccessRight.PROP_ACCESS_TO);
		// TODO PROP_ACCESS_TO should be restricted to a TypeExpression, which
		// should be in turn relevant only to Asset type. Cardinality should be
		// 1, 1.
		// oci_AccessRight.addRestriction(MergedRestriction.getAllValuesRestrictionWithCardinality(AccessRight.PROP_ACCESS_TO,
		// TypeExpression., 1, 1));
		...
		oci_delegationForm.setResourceComment(
				"Delegation Form is a SignedResource, signed by the Authoriser enabling the Delegate to perform some Competences in the form of Roles.");
		oci_delegationForm.addObjectProperty(DelegationForm.PROP_AUTHORISER);
		oci_delegationForm.addRestriction(MergedRestriction
				.getAllValuesRestrictionWithCardinality(DelegationForm.PROP_AUTHORISER, User.MY_URI, 1, 1));
		oci_delegationForm.addObjectProperty(DelegationForm.PROP_DELEGATE);
		oci_delegationForm.addRestriction(MergedRestriction
				.getAllValuesRestrictionWithCardinality(DelegationForm.PROP_DELEGATE, User.MY_URI, 1, 1));
		oci_delegationForm.addObjectProperty(DelegationForm.PROP_DELEGATED_COMPETENCES);
		oci_delegationForm.addRestriction(MergedRestriction
				.getAllValuesRestrictionWithCardinality(DelegationForm.PROP_DELEGATED_COMPETENCES, Role.MY_URI, 1, -1));
		// Default AccessRight on DelegationForm
		// AccessRight ddfar = new
		// AccessRight(NAMESPACE+"defaultAllReadDelegationForm");
		// ddfar.addAccessType(AccessType.read);
		// TypeExpression te = new TypeURI(DelegationForm.MY_URI, false);
		// ddfar.setAccessTo(te);
		// oci_delegationForm.addRestriction(MergedRestriction.getFixedValueRestriction(Asset.PROP_HAS_DEFAULT_ACCESS,
		// ddfar));
		...
		oci_devSession.addObjectProperty(DeviceBoundSession.PROP_BOUNDED_DEVICE)
				.addSuperProperty(Session.PROP_BOUND_TO);
		oci_devSession.addRestriction(MergedRestriction
				.getAllValuesRestrictionWithCardinality(DeviceBoundSession.PROP_BOUNDED_DEVICE, Device.MY_URI, 1, 1));
		...
		oci_locSession.addObjectProperty(LocationBoundSession.PROP_BOUNDED_LOCATION)
				.addSuperProperty(Session.PROP_BOUND_TO);
		oci_locSession.addRestriction(MergedRestriction.getAllValuesRestrictionWithCardinality(
				LocationBoundSession.PROP_BOUNDED_LOCATION, AbsLocation.MY_URI, 1, 1));

		// SecurityProfile
		oci_secProf.setResourceComment(
				"Profile storing credentials, Roles, AcessRights and any security concept for a given profilable.");
		...
		oci_secProf.addRestriction(
				MergedRestriction.getAllValuesRestriction(SecuritySubprofile.PROP_CREDENTIALS, Credentials.MY_URI));
		oci_secProf.addObjectProperty(SecuritySubprofile.PROP_ROLES);
		oci_secProf
				.addRestriction(MergedRestriction.getAllValuesRestriction(SecuritySubprofile.PROP_ROLES, Role.MY_URI));
		oci_secProf.addObjectProperty(SecuritySubprofile.PROP_DELEGATED_FORMS);
		oci_secProf.addRestriction(MergedRestriction.getAllValuesRestriction(SecuritySubprofile.PROP_DELEGATED_FORMS,
				DelegationForm.MY_URI));
		...
		oci_user.addRestriction(
				MergedRestriction.getAllValuesRestrictionWithCardinality(PROP_SESSION, Session.MY_URI, 0, 1));
		// USER.authenticatedTo
		oci_user.addObjectProperty(PROP_AUTHENTICATED);
		oci_user.addRestriction(
				MergedRestriction.getAllValuesRestrictionWithCardinality(PROP_AUTHENTICATED, Device.MY_URI, 0, 1));
		// USER.deauthenticatedby
		oci_user.addObjectProperty(PROP_REVOKED);
		oci_user.addRestriction(
				MergedRestriction.getAllValuesRestrictionWithCardinality(PROP_REVOKED, Device.MY_URI, 0, 1));

		/*
		 * Services
		 */
		// Authentication Service
		oci_authService.setResourceComment("Service to Authenticate a User Given some credentials.");
		...
		oci_authService.addRestriction(MergedRestriction.getAllValuesRestrictionWithCardinality(
				AuthenticationService.PROP_GIVEN_CREDENTIALS, Credentials.MY_URI, 1, 1));
		oci_authService.addObjectProperty(AuthenticationService.PROP_AUTHENTICATED_USER);
		oci_authService.addRestriction(MergedRestriction.getAllValuesRestrictionWithCardinality(
				AuthenticationService.PROP_AUTHENTICATED_USER, User.MY_URI, 0, 1));
		...
		oci_roleMngService.addRestriction(
				MergedRestriction.getAllValuesRestriction(RoleManagementService.PROP_ROLE, Role.MY_URI));
		...
		oci_authorizationService.addRestriction(MergedRestriction
				.getAllValuesRestrictionWithCardinality(AuthorizationService.PROP_CHALLENGER_USER, User.MY_URI, 0, 1));
		oci_authorizationService.addObjectProperty(AuthorizationService.PROP_ASSET_ACCESS);
		oci_authorizationService.addRestriction(MergedRestriction
				.getAllValuesRestrictionWithCardinality(AuthorizationService.PROP_ASSET_ACCESS, Asset.MY_URI, 1, 1));
		...
		oci_anonymizationService.addRestriction(
				MergedRestriction.getAllValuesRestriction(AnonymizationService.PROP_ANONYMIZABLE, Anonymizable.MY_URI));
		oci_anonymizationService.addObjectProperty(AnonymizationService.PROP_ASYMMETRIC_ENCRYPTION).setFunctional();
		oci_anonymizationService.addRestriction(MergedRestriction
				.getAllValuesRestriction(AnonymizationService.PROP_ASYMMETRIC_ENCRYPTION, AsymmetricEncryption.MY_URI));
		...
		oci_sessionService.addRestriction(
				MergedRestriction.getAllValuesRestriction(SessionManagementService.PROP_USER, User.MY_URI));
