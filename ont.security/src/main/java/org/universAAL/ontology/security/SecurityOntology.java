...
public final class SecurityOntology extends Ontology {
	...
	public void create() {
		...
		OntClassInfoSetup oci_credentials = createNewOntClassInfo(
				Credentials.MY_URI, factory, 16);

		OntClassInfoSetup oci_session = createNewOntClassInfo(Session.MY_URI,
				factory, 17);
		...
		OntClassInfoSetup oci_anonymizable = createNewOntClassInfo(
				Anonymizable.MY_URI, factory, 15);
		oci_anonymizable.addSuperClass(ManagedIndividual.MY_URI);
		oci_anonymizable.setResourceLabel("Anonymizable");
		oci_anonymizable
				.setResourceComment("Any anonymizable resource should be subclass of this class to enable the anonymization service to encrypt the property reference");
		oci_anonymizable.addObjectProperty(
				Anonymizable.PROP_ANNONYMOUS_RESOURCE).setFunctional();
		...
		OntClassInfoSetup oci_passwd = createNewOntClassInfo(Password.MY_URI,
				factory, 7);
		OntClassInfoSetup oci_otp = createNewOntClassInfo(
				OneTimePassword.MY_URI, factory, 8);
		OntClassInfoSetup oci_role = createNewOntClassInfo(Role.MY_URI,
				factory, 9);
		OntClassInfoSetup oci_AccessRight = createNewOntClassInfo(
				AccessRight.MY_URI, factory, 10);
		OntClassInfoSetup oci_authorizationService = createNewOntClassInfo(
				AuthorizationService.MY_URI, factory, 11);
		OntClassInfoSetup oci_delegationForm = createNewOntClassInfo(
				DelegationForm.MY_URI, factory, 12);
		OntClassInfoSetup oci_roleMngService = createNewOntClassInfo(
				RoleManagementService.MY_URI, factory, 13);
		OntClassInfoSetup oci_anonymizationService = createNewOntClassInfo(
				AnonymizationService.MY_URI, factory, 14);
		...
		oci_passwd.addRestriction(MergedRestriction
				.getAllValuesRestrictionWithCardinality(
						Password.PROP_PASSWORD_DIGEST, Digest.MY_URI, 0, 1));
		...
		oci_asset
				.setResourceComment("Mark a class or an instance as a security Asset to be access through the authorisation service");
		oci_asset.addObjectProperty(Asset.PROP_HAS_DEFAULT_ACCESS);
		oci_asset
				.addRestriction(MergedRestriction
						.getAllValuesRestrictionWithCardinality(
								Asset.PROP_HAS_DEFAULT_ACCESS,
		...
		oci_role.addRestriction(MergedRestriction.getAllValuesRestriction(
				Role.PROP_SUB_ROLES, Role.MY_URI));
		oci_role.addObjectProperty(Role.PROP_HAS_ACCESS_RIGHTS);
		oci_role.addRestriction(MergedRestriction.getAllValuesRestriction(
				Role.PROP_HAS_ACCESS_RIGHTS, AccessRight.MY_URI));

		// Access Type
		oci_accessType.setResourceLabel("Access Type");
		oci_accessType
				.setResourceComment("The different Access Types, Read, Write (change or add), delete (remove).");
		oci_accessType.toEnumeration(new ManagedIndividual[] { AccessType.read,
				AccessType.change, AccessType.add, AccessType.remove });

		// Access Right
		oci_AccessRight.addSuperClass(ManagedIndividual.MY_URI);
		oci_AccessRight.setResourceLabel("Access Right");
		oci_AccessRight
				.setResourceComment("Access Right describes the type of access right to a specific (or generic) class (or instance) of Asset(s).");
		oci_AccessRight.addObjectProperty(AccessRight.PROP_ACCESS_TYPE);
		oci_AccessRight.addRestriction(MergedRestriction
				.getAllValuesRestrictionWithCardinality(
						AccessRight.PROP_ACCESS_TYPE, AccessType.MY_URI, 1, 4));
		...
		oci_delegationForm
				.setResourceComment("Delegation Form is a SignedResource, signed by the Authoriser enabling the Delegate to perform some Competences in the form of Roles.");
		oci_delegationForm.addObjectProperty(DelegationForm.PROP_AUTHORISER);
		oci_delegationForm.addRestriction(MergedRestriction
				.getAllValuesRestrictionWithCardinality(
						DelegationForm.PROP_AUTHORISER, User.MY_URI, 1, 1));
		oci_delegationForm.addObjectProperty(DelegationForm.PROP_DELEGATE);
		oci_delegationForm.addRestriction(MergedRestriction
				.getAllValuesRestrictionWithCardinality(
						DelegationForm.PROP_DELEGATE, User.MY_URI, 1, 1));
		oci_delegationForm
				.addObjectProperty(DelegationForm.PROP_DELEGATED_COMPETENCES);
		oci_delegationForm.addRestriction(MergedRestriction
				.getAllValuesRestrictionWithCardinality(
						DelegationForm.PROP_DELEGATED_COMPETENCES, Role.MY_URI,
						1, -1));
		...
		oci_secProf
				.addRestriction(MergedRestriction.getAllValuesRestriction(
						SecuritySubprofile.PROP_DELEGATED_FORMS,
		...
		oci_roleMngService
				.setResourceComment("Service to Manage Roles and Access Rights within roles");
		...
		oci_roleMngService.addRestriction(MergedRestriction
				.getAllValuesRestriction(RoleManagementService.PROP_ROLE,
						Role.MY_URI));

		// Authorization Service
		oci_authorizationService
				.setResourceComment("Service to Authorise a User Given a specific acces to an Asset.");
		oci_authorizationService.setResourceLabel("Authorisation Service");
		oci_authorizationService.addSuperClass(Service.MY_URI);
		oci_authorizationService
				.addObjectProperty(AuthorizationService.PROP_CHALLENGER_USER);
		oci_authorizationService.addRestriction(MergedRestriction
				.getAllValuesRestrictionWithCardinality(
						AuthorizationService.PROP_CHALLENGER_USER, User.MY_URI,
						0, 1));
		oci_authorizationService
				.addObjectProperty(AuthorizationService.PROP_ASSET_ACCESS);
		oci_authorizationService.addRestriction(MergedRestriction
				.getAllValuesRestrictionWithCardinality(
						AuthorizationService.PROP_ASSET_ACCESS, Asset.MY_URI,
						1, 1));

		// Anonymization Service
		oci_anonymizationService
				.setResourceComment("Service To (de)anonymize references to objects");
		oci_anonymizationService.setResourceLabel("Anonymization Service");
		oci_anonymizationService.addSuperClass(Service.MY_URI);
		oci_anonymizationService.addObjectProperty(
				AnonymizationService.PROP_ANONYMIZABLE).setFunctional();
		oci_anonymizationService.addRestriction(MergedRestriction
				.getAllValuesRestriction(
						AnonymizationService.PROP_ANONYMIZABLE,
						Anonymizable.MY_URI));
		oci_anonymizationService.addObjectProperty(
				AnonymizationService.PROP_ASYMMETRIC_ENCRYPTION)
				.setFunctional();
		oci_anonymizationService.addRestriction(MergedRestriction
				.getAllValuesRestriction(
						AnonymizationService.PROP_ASYMMETRIC_ENCRYPTION,
						AsymmetricEncryption.MY_URI));
