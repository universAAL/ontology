...
public class ConeSegment extends LineSegment {
	...
	public static Restriction getClassRestrictionsOnProperty(String propURI) {
		if (PROP_RADIUS_END.equals(propURI) || PROP_RADIUS_START.equals(propURI))
	...
	public float getDistanceTo(Point point) {
		...
		for(int i=0;i<proj.length;i++) {
			...
			if((curdist = proj[i].distance(c)-radiusAt(t)) < dist) dist = (float)curdist;
		}
		return dist;
	}
	...
	private float distanceRay(Point start,Point dir,Box box,float minDir, float maxDir) {
		...
		CoordinateSystem common = CoordinateSystem.findCommonParentSystem(start.getCoordinateSystem(), box.getLocalCoordinateSystem());
		dir = dir.getInHigherCoordinateSystem(common);
		start = start.getInHigherCoordinateSystem(common);
		
		Point invDir = new Point((float)(1f / dir.getX()),(float)( 1f / dir.getY()),(float)(1f / dir.getZ()),new CoordinateSystem());
		...
		//TODO: is the association between width, depth, height and x,y,z right at this place?
		...
		//
		float dist =  (float)(Math.sqrt(Math.pow(pt[0],2)+Math.pow(pt[1],2)+Math.pow(pt[2],2))-radiusAt((float)tmin));
