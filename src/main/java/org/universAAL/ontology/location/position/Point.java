...
public class Point extends Location {
	 ...
	 * @param y
	 * @param z
	 * @param system the coordinate system of the point
	 */
	...
	/**
	 * 
	 * @return the double array {x, y, z}.If the point z property is missing the method assumes that it is zero.
	 */
	public double [] get3DCoordinates(){
		double z=0;
		if (props.containsKey(PROP_Z))
			z = ((Double)props.get(PROP_Z)).doubleValue();
		double [] get3DCoordinates = {((Double)props.get(PROP_X)).doubleValue(), 
	...
	public double [] get2DCoordinates(){
		double [] get2DCoordinates = {((Double)props.get(PROP_X)).doubleValue(), ((Double)props.get(PROP_Y)).doubleValue()};
	...
	public boolean isWellFormed() {
		return props.containsKey(PROP_X)&&props.containsKey(PROP_Y)&&props.containsKey(PROP_COORDINATE_SYSTEM);
	}

	/**
	 * Both points have to be in the same CoordinateSystem
	 * If the System is WGS84, a point in latitude/longitude is assume
	 * If not, the euclidean distance is returned.
	 */
	public float getDistanceTo(Location other) {
		if(other == null || other.getClass()!=Point.class)
			throw new IllegalArgumentException();
		CoordinateSystem commonsys = CoordinateSystem.findCommonParentSystem(getCoordinateSystem(), ((Point)other).getCoordinateSystem());
		...
		if(p1.getCoordinateSystem() == p2.getCoordinateSystem()) {
			if(this.getCoordinateSystem().getURI().indexOf("WGS84") > -1) {
				...
				double[] SolVec = {otherCoords[0]-thisCoords[0],otherCoords[1]-thisCoords[1],otherCoords[2]-thisCoords[2]};
	 ...
	 */
	private double getDistanceFromLatLong(double[] current2DCoordinatesDeg, double[] other2DCoordinatesDeg)
	{
	...
	/**
	 * if the point is in a OriginedMetric, the point will be transformed to the next higher system
	 * A 2d point gets transformed to a 3d point, because all non local coordinate systems are 3d
	 * @return the point
	 */
	public Point getInHigherCoordinateSystem() {
		/*CoordinateSystem Sys = getCoordinateSystem();
		if(!(Sys instanceof OriginedMetric)) throw new IllegalArgumentException("Point is not in an OriginedMetric");
		Point origin = ((OriginedMetric)Sys).getOrigin();
		if(Sys.getClass() != origin.getCoordinateSystem().getClass()) throw new IllegalArgumentException("Higher system is of an other type");
		double[] p = get3DCoordinates();
		double[] o = origin.get3DCoordinates();
		if(Double.isNaN(p[2])) p[2] = 0;
		...
		map.transform(point);
		return new Point(point.x,point.y,point.z,origin.getCoordinateSystem());*/
		try {
			throw new Exception("The method 'getInHigherCoordinateSystem' is disabled in this version of casf.ont to " +
			...
			System.out.println("The method 'getInHigherCoordinateSystem' is disabled in this version of casf.ont to " +
					"avoid GPL libraries. If you really need this method, use the restricted GPL version, or" +
					"modify source code of this version to include the dependencies you need:" +
					"j3d-core and vecmath, versions at least 1.3.1.");
			e.printStackTrace();
	...
	/**
	 * if the point is in a OriginedMetric, the point will be transformed to the given higher system
	 * A 2d point gets transformed to a 3d point, because all non local coordinate systems are 3d
	 * @param system the target system
	 * @return the point or null if the target system is not part of the higher coordinate systems chain
	 */
	...
	public String toString() {
		return "["+props.get(PROP_X)+","+props.get(PROP_Y)+","+props.get(PROP_Z)+","+getCoordinateSystem()+"]";
	}
	...
	public void castTo3D() {
		if(is2D()) props.put(PROP_Z, new Double(0));
	}
